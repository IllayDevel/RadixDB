...
//! Fair benchmark comparison: RadixDB vs SQLite for DELETE by ID
//!
...
/// Setup RadixDB database with test data
fn setup_radixdb() -> Database {
    ...
    insert_radixdb_data(&db);
    db
}

fn insert_radixdb_data(db: &Database) {
    let insert_stmt = db
...
fn bench_delete_by_id(c: &mut Criterion) {
    ...
    // Setup databases
    let radixdb_db = setup_radixdb();
    #[cfg(feature = "sqlite")]
    let sqlite_conn = setup_sqlite();

    // Prepare statements
    let radixdb_delete = radixdb_db
        .prepare("DELETE FROM users WHERE id = $1")
        .unwrap();
    let radixdb_insert = radixdb_db
        .prepare("INSERT INTO users (id, name, email, age, balance, active, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)")
        .unwrap();
    ...
    // Benchmark RadixDB - delete then re-insert to keep data available
    group.bench_function("radixdb", |b| {
        let mut idx = 0;
        b.iter(|| {
            ...
            // Delete the row
            radixdb_delete.execute(black_box((id,))).unwrap();
...
fn bench_delete_by_id_only(c: &mut Criterion) {
    ...
    // Benchmark RadixDB - fresh database per batch, delete 100 rows
    group.bench_function("radixdb", |b| {
        b.iter_batched(
            || {
