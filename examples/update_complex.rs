...
//! Fair benchmark comparison: RadixDB vs SQLite for complex UPDATE queries
//!
...
fn bench_update_with_exists(c: &mut Criterion) {
    ...
    group.bench_function("radixdb", |b| {
        ...
        b.iter(|| {
            ...
            iter_count += 1;
            radixdb_stmt.execute(black_box((bonus,))).unwrap();
            black_box(())
        });
    });
...
fn bench_update_full_scan_small(c: &mut Criterion) {
    ...
    group.bench_function("radixdb", |b| {
        ...
        b.iter(|| {
            let new_balance = (iter_count as f64) * 10.0;
            iter_count += 1;
            radixdb_stmt.execute(black_box((new_balance,))).unwrap();
            black_box(())
        });
    });
