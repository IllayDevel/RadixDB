...
//! SQLite benchmark matching the RadixDB benchmark format
//!
...
fn main() {
    ...
    // Populate data
    {
        ...
        for i in 1..=ROW_COUNT {
            ...
            let active: i32 = if rng.random_bool(0.7) { 1 } else { 0 };
            insert_stmt
                .execute(params![
                    i as i64,
                    format!("User_{}", i),
    ...
    {
        let mut stmt = conn.prepare("SELECT * FROM users WHERE age = ?1").unwrap();
    ...
    // UPDATE by ID
    {
        let mut stmt = conn
            ...
            .unwrap();
        let params: Vec<(f64, i64)> = (0..ITERATIONS)
            .map(|i| {
                (
    ...
    // UPDATE complex (same range as RadixDB: age 27-28)
    {
        let mut stmt = conn
    ...
    // INSERT single
    {
        let mut stmt = conn
            .prepare("INSERT INTO users (id, name, email, age, balance, active, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)")
            .unwrap();
        let params: Vec<(i64, String, String, i64)> = (0..ITERATIONS)
            .map(|i| {
            ...
            .collect();
        let start = Instant::now();
        for (id, name, email, age) in &params {
            stmt.execute(params![
                id,
                name,
                email,
    ...
    // DELETE complex (same range as RadixDB: age 25-26)
    {
        let mut stmt = conn
            .prepare("DELETE FROM users WHERE age >= ?1 AND age <= ?2 AND active = 1")
            .unwrap();
        let start = Instant::now();
        for _ in 0..ITERATIONS {
    ...
    // Create orders table (same as RadixDB)
    conn.execute(
        "CREATE TABLE orders (
            id INTEGER PRIMARY KEY,
    ...
    // Window ROW_NUMBER (PK) - same as RadixDB
    {
        let mut stmt = conn
    ...
    {
        let mut stmt = conn
            .prepare("SELECT u.name, COUNT(DISTINCT o.id) as orders, SUM(o.amount) as total FROM users u INNER JOIN orders o ON u.id = o.user_id WHERE u.active = 1 AND o.status IN ('completed', 'shipped') GROUP BY u.id, u.name HAVING COUNT(o.id) > 1 LIMIT 50")
            .unwrap();
        let start = Instant::now();
        for _ in 0..20 {
            let mut rows = stmt.query([]).unwrap();
    ...
    {
        let mut stmt = conn.prepare("SELECT name, COALESCE(balance, 0) as bal FROM users WHERE balance IS NOT NULL LIMIT 100").unwrap();
        let start = Instant::now();
        for _ in 0..ITERATIONS {
            let mut rows = stmt.query([]).unwrap();
            while rows.next().unwrap().is_some() {}
    ...
    // Expr in WHERE (funcs)
    {
        let mut stmt = conn.prepare("SELECT * FROM users WHERE LENGTH(name) > 7 AND UPPER(name) LIKE 'USER_%' LIMIT 100").unwrap();
        let start = Instant::now();
        for _ in 0..ITERATIONS {
            let mut rows = stmt.query([]).unwrap();
            while rows.next().unwrap().is_some() {}
    ...
    {
        let mut stmt = conn
            .prepare("SELECT id, name, balance FROM users WHERE active = 1")
            .unwrap();
        let start = Instant::now();
        for _ in 0..20 {
            let mut rows = stmt.query([]).unwrap();
    ...
    {
        let mut stmt = conn.prepare("SELECT * FROM users WHERE balance > (SELECT AVG(amount) * 100 FROM orders) LIMIT 100").unwrap();
        let start = Instant::now();
        for _ in 0..ITERATIONS {
            let mut rows = stmt.query([]).unwrap();
            while rows.next().unwrap().is_some() {}
