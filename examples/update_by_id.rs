...
//! Fair benchmark comparison: RadixDB vs SQLite for UPDATE by ID
//!
//! Run with: cargo bench --bench update_by_id
//! Run with SQLite comparison: cargo bench --bench update_by_id --features sqlite
//!
...
fn bench_update_by_id(c: &mut Criterion) {
    ...
    group.bench_function("radixdb", |b| {
        ...
        b.iter(|| {
            ...
            idx += 1;
            radixdb_stmt.execute(black_box((new_balance, id))).unwrap();
            black_box(())
        });
    });
...
fn bench_update_by_id_batch(c: &mut Criterion) {
    ...
    // Benchmark RadixDB - 100 updates per iteration
    group.bench_function("radixdb", |b| {
