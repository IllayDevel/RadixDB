...
//!
//! Measures real end-to-end performance of RadixDB's vector search through SQL:
...
use radixdb::storage::index::{default_ef_construction, default_ef_search, default_m_for_dims};
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                println!("RadixDB Vector Search Performance Benchmark");
                println!();
    ...
    println!();
    print_separator(76);
    println!("  RADIXDB VECTOR SEARCH BENCHMARK (End-to-End SQL)");
    print_separator(76);
    println!();
