...
//! Fair benchmark comparison: RadixDB vs SQLite for SELECT by ID
//!
...
fn bench_select_by_id(c: &mut Criterion) {
    ...
    group.bench_function("radixdb", |b| {
        ...
        b.iter(|| {
            ...
            idx += 1;
            let rows = radixdb_stmt.query(black_box((id,))).unwrap();
...
fn bench_select_by_id_batch(c: &mut Criterion) {
    ...
    // Benchmark RadixDB - 100 queries per iteration
    group.bench_function("radixdb", |b| {
        b.iter(|| {
            for &id in &ids {
                let rows = radixdb_stmt.query(black_box((id,))).unwrap();
...
fn bench_select_by_id_with_result_processing(c: &mut Criterion) {
    ...
    // Benchmark RadixDB - read all columns
    group.bench_function("radixdb", |b| {
        let mut idx = 0;
        b.iter(|| {
            let id = ids[idx % ids.len()];
