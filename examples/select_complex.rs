...
//! Fair benchmark comparison: RadixDB vs SQLite for complex SELECT queries
//!
...
fn bench_select_complex(c: &mut Criterion) {
    ...
    group.bench_function("radixdb", |b| {
        b.iter(|| {
            let rows = radixdb_stmt.query(()).unwrap();
            for row in rows {
...
fn bench_full_scan(c: &mut Criterion) {
    ...
    let radixdb_stmt = radixdb_db.prepare("SELECT * FROM users").unwrap();
