...
fn test_explain_analyze_filter_on_correct_table() {
    ...
    assert_no_filter_under_scan(&lines, idx, "o.status", &plan);
    assert!(
        plan.contains("Physical JOIN Summary: operators=1"),
        "EXPLAIN ANALYZE must expose request-local physical JOIN work:\n{plan}"
    );
    assert!(
        plan.contains("Physical JOIN Root Edge:")
            && plan.contains("left_rows=1")
            && plan.contains("output_rows=1"),
        "EXPLAIN ANALYZE must prove the filtered root reached the first JOIN edge:\n{plan}"
    );
    assert!(
        plan.contains("Physical JOIN Phases:")
            && plan.contains("logical_planning=")
            && plan.contains("physical_planning=")
            && plan.contains("executor_wall=")
            && plan.contains("operator_wall_sum_non_additive=")
            && plan.contains("query_wall="),
        "EXPLAIN ANALYZE must split planning and execution phases:\n{plan}"
    );
    assert!(
        plan.contains("Protocol Boundary: excluded_from_embedded_explain=true"),
        "embedded EXPLAIN must state where protocol wall is measured:\n{plan}"
    );
    assert!(
        plan.contains("Physical JOIN Memory:")
            && plan.contains("request_peak_bytes=")
            && plan.contains("retained_after_drain_bytes=0"),
        "EXPLAIN ANALYZE must expose bounded JOIN ownership and release:\n{plan}"
    );
    assert!(
        plan.contains("Physical JOIN Actual[0]:")
            && plan.contains("stages_ms=outer:")
            && !plan.contains("truncated_operators="),
        "EXPLAIN ANALYZE must retain a bounded per-operator trace:\n{plan}"
    );
}

#[test]
fn explain_join_edges_have_stable_operator_parent_and_relation_identity() {
    let db = setup_db();
    db.execute(
    ...
    let sql = "EXPLAIN SELECT p.id FROM users u \
               JOIN orders o ON o.user_id = u.id \
               LEFT JOIN payments p ON p.order_id = o.id";
    let first = get_explain_lines(&db, sql).join("\n");
    let second = get_explain_lines(&db, sql).join("\n");
    assert_eq!(
        first, second,
        "physical edge identities must be deterministic"
    );
    assert!(
        first.contains("Physical Edge Identity: operator_id=0, parent_id=none, left_relations=[o,u], right_relations=[p], barrier=left"),
        "root edge identity is missing:\n{first}"
    );
    assert!(
        first.contains("Physical Edge Identity: operator_id=1, parent_id=0, left_relations=[u], right_relations=[o], barrier=reorderable_inner"),
        "child edge identity is missing:\n{first}"
    );
}
