...
#[test]
fn r8_l01_batch_a_isolation_capabilities_are_asserted() {
    ...
    // SERIALIZABLE is deliberately rejected rather than silently downgraded.
    let error = db
        .execute("SET ISOLATIONLEVEL = 'SERIALIZABLE'", ())
        .expect_err("SERIALIZABLE must not be accepted without its full contract");
    assert!(
        error.to_string().contains("unsupported isolation level"),
        "unexpected SERIALIZABLE rejection: {error}"
    );
}
