...
use radixdb::{Database, DEFAULT_WAL_MAX_SIZE};
use std::fs;
use std::path::Path;
...
#[test]
fn r8_l01_batch_b_wal_truncates_at_observed_checkpoints() {
    ...
    let inserted_by_writer = Arc::clone(&inserted);
    ...
    let writer = thread::spawn(move || {
        ...
        while !stop_clone.load(Ordering::Relaxed) {
            ...
            if wdb
                ...
                .is_ok()
            {
                inserted_by_writer.store(i as u64, Ordering::Release);
            }
            if i % 100 == 0 {
    ...
    // Bound the concurrent phase by completed work rather than wall time.
    let deadline = std::time::Instant::now() + Duration::from_secs(12);
    while inserted.load(Ordering::Acquire) < 5_000 {
        assert!(
            std::time::Instant::now() < deadline,
            "writer did not complete the declared workload before the deadline"
        );
        thread::yield_now();
    }
    ...
    // Run an explicit checkpoint after the writer stops. Retention can remove
    // only whole generations and may therefore keep the active generation.
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();

    // The active generation remains bounded by the configured rotation cap;
    // checkpoint is not allowed to rewrite/truncate it in place.
    let wal_size = wal_dir_total_size(&db_path);
    assert!(
        wal_size <= DEFAULT_WAL_MAX_SIZE,
        "WAL generations exceeded the configured {} byte cap: got {} bytes \
         after {} inserted rows",
    ...
    // Every successful auto-commit remains visible before and after reopen.
    let count = query_i64(&db, "SELECT COUNT(*) FROM writes");
    assert_eq!(count, rows_inserted);

    db.close().unwrap();

    let db = Database::open(&dsn).unwrap();
    let reopened_count = query_i64(&db, "SELECT COUNT(*) FROM writes");
    assert_eq!(reopened_count, rows_inserted);
    db.close().unwrap();
}
...
// ---------------------------------------------------------------------------
// 5. Read-only reopen does not append another catalog generation
// ---------------------------------------------------------------------------

#[test]
fn test_read_only_reopen_keeps_catalog_wal_bounded() {
    let dir = tempfile::tempdir().unwrap();
    let db_path = dir.path().join("wal_reopen_catalog_bound");
    let dsn = format!(
        "file://{}?checkpoint_interval=3600&sync_mode=normal",
        db_path.display()
    );

    {
        let db = Database::open(&dsn).unwrap();
        db.execute(
            "CREATE TABLE items (id INTEGER PRIMARY KEY, category INTEGER, value TEXT)",
            (),
        )
        .unwrap();
        db.execute("CREATE INDEX idx_items_category ON items(category)", ())
            .unwrap();
        db.execute(
            "INSERT INTO items VALUES (1, 7, 'one'), (2, 7, 'two'), (3, 9, 'three')",
            (),
        )
        .unwrap();
        db.close().unwrap();
    ...
    for _ in 0..4 {
        ...
        assert_eq!(
            query_i64(&db, "SELECT COUNT(*) FROM items WHERE category = 7"),
            2
        );
        db.close().unwrap();
        sizes.push(wal_dir_total_size(&db_path));
    }

    let min_size = *sizes.iter().min().unwrap();
    let max_size = *sizes.iter().max().unwrap();
    assert!(
        max_size.saturating_sub(min_size) <= 512,
        "read-only reopen appended duplicate catalog WAL: sizes={sizes:?}"
    );
}
