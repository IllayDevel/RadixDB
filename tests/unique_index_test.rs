...
#[test]
fn r6_unique_if_not_exists_does_not_accept_unrelated_column_coverage() {
    let db = Database::open("memory://unique_if_not_exists_definition").expect("database");
    db.execute("CREATE TABLE tags (id INTEGER PRIMARY KEY, tag TEXT)", ())
        .unwrap();
    db.execute("CREATE INDEX idx_tag_lookup ON tags(tag)", ())
        .unwrap();
    ...
    let error = db
        ...
        .expect_err(
            "an unrelated non-unique index must not masquerade as the requested unique one",
        );
    assert!(error.to_string().contains("non-unique index"), "{error}");

    db.execute("INSERT INTO tags VALUES (1, 'same')", ())
        .unwrap();
    db.execute("INSERT INTO tags VALUES (2, 'same')", ())
        .expect("the failed definition must not invent a unique constraint");

    db.execute("DROP INDEX idx_tag_lookup ON tags", ()).unwrap();
    assert!(
        db.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_tag_unique ON tags(tag)",
            (),
        ...
        .is_err(),
        "existing duplicate data must now be validated"
    );
    db.execute("DELETE FROM tags WHERE id = 2", ()).unwrap();
    db.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_tag_unique ON tags(tag)",
        (),
    )
    .unwrap();
    assert!(db
        .execute("INSERT INTO tags VALUES (3, 'same')", ())
        .is_err());

    // IF NOT EXISTS is a no-op only for the same normalized definition under
    // the requested name, never for a conflicting definition.
    assert!(db
        .execute("CREATE INDEX IF NOT EXISTS idx_tag_unique ON tags(tag)", (),)
        .is_err());
}
