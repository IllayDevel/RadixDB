...
#[test]
fn r6_update_rejects_the_whole_assignment_set_on_late_compile_error() {
    let db = Database::open("memory://update_assignment_compile_error").expect("database");
    db.execute(
        "CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT, score INTEGER)",
        (),
    )
    .unwrap();
    db.execute("INSERT INTO items VALUES (1, 'before', 7)", ())
        .unwrap();

    let error = db
        .execute(
            "UPDATE items \
             SET name = 'silently-applied', score = definitely_missing(score) \
             WHERE id = 1",
            (),
        )
        .expect_err("one invalid assignment must reject the statement");
    assert!(
        error
            .to_string()
            .to_lowercase()
    ...
    let row = db
        .query("SELECT name, score FROM items WHERE id = 1", ())
        .unwrap()
        .next()
        .unwrap()
        .unwrap();
    assert_eq!(row.get::<String>(0).unwrap(), "before");
