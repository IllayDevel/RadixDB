...
use radixdb::storage::v6::{
    decode_database_manifest, decode_table_manifest, select_control_slots, TableManifest,
...
fn selected_control(db_path: &Path) -> radixdb::storage::v6::ControlRecord {
    let read_slot = |name: &str| {
        fs::read(db_path.join(name)).unwrap_or_else(|_| vec![0_u8; CONTROL_RECORD_BYTES])
    };
    let control_0 = read_slot("CONTROL.0");
    let control_1 = read_slot("CONTROL.1");
    select_control_slots(&control_0, &control_1, |_| true).unwrap()
}

fn read_only_table_manifest(db_path: &Path) -> (PathBuf, TableManifest) {
    let control = selected_control(db_path);
    let database_manifest_path = db_path.join("manifests").join(format!(
        "database-{:016x}.mft",
        control.database_manifest().generation().get()
    ));
    let database_manifest =
        decode_database_manifest(&fs::read(database_manifest_path).unwrap()).unwrap();
    assert_eq!(
        database_manifest.tables().len(),
        1,
        "test fixture must contain exactly one table"
    );
    let reference = database_manifest.tables()[0];
    let path = db_path
        .join("manifests")
        .join("tables")
    ...
    let manifest = decode_table_manifest(&fs::read(&path).unwrap()).unwrap();
    (path, manifest)
}

fn list_active_data_artifacts(db_path: &Path) -> Vec<PathBuf> {
    let (_, manifest) = read_only_table_manifest(db_path);
    manifest
        .segments()
        .iter()
        .map(|segment| db_path.join(segment.data_artifact().relative_path()))
        .collect()
}
...
#[test]
fn test_snapshot_transaction_defers_background_compaction() {
    let dir = tempfile::tempdir().unwrap();
    ...
    assert_eq!(list_active_data_artifacts(&db_path).len(), 3);
    ...
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();
    // CHECKPOINT publishes durability and requests independent background
    // compaction. Physical maintenance must leave the generation untouched
    // while a Snapshot still needs its row-level MVCC provenance.
    std::thread::sleep(Duration::from_millis(250));
    assert_eq!(
        list_active_data_artifacts(&db_path).len(),
        3,
        "background compaction must defer while a snapshot is active"
    );

    // The deferred rewrite must not invalidate or change the live reader.
    let count_after: i64 = snapshot_tx.query_one("SELECT COUNT(*) FROM t", ()).unwrap();
    assert_eq!(
        count_after, 30,
        "snapshot data integrity while compaction defers"
    );

    snapshot_tx.rollback().unwrap();

    // Re-arm maintenance after releasing the snapshot. The same three inputs
    // must now merge without requiring a close/reopen cycle.
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();
    let deadline = Instant::now() + Duration::from_secs(5);
    while list_active_data_artifacts(&db_path).len() != 1 && Instant::now() < deadline {
        std::thread::sleep(Duration::from_millis(10));
    }
    assert_eq!(
        list_active_data_artifacts(&db_path).len(),
        1,
        "background compaction should finish for pre-snapshot volumes"
    );
...
#[test]
fn test_startup_rejects_missing_manifest_segment_before_compaction() {
    let dir = tempfile::tempdir().unwrap();
    ...
    let data_artifacts = list_active_data_artifacts(&db_path);
    assert_eq!(
        data_artifacts.len(),
        2,
        "expected two active DATA artifacts"
    );

    let (manifest_path, manifest_before) = read_only_table_manifest(&db_path);
    let manifest_bytes_before = fs::read(&manifest_path).unwrap();
    assert_eq!(manifest_before.segments().len(), 2);
    let segment_ids_before: Vec<_> = manifest_before
        .segments()
        .iter()
        .map(|segment| segment.id())
        .collect();

    fs::remove_file(&data_artifacts[0]).unwrap();
    ...
    assert!(
        Database::open(&compact_dsn).is_err(),
        "startup published a table with a missing required manifest segment"
    );

    let (manifest_path_after, manifest_after) = read_only_table_manifest(&db_path);
    let segment_ids_after: Vec<_> = manifest_after
        .segments()
        .iter()
        .map(|segment| segment.id())
        .collect();
    ...
    assert_eq!(
        ...
        2,
        "failed startup must not rewrite the manifest"
    );
    assert_eq!(
        segment_ids_after, segment_ids_before,
        "failed startup must preserve manifest membership for diagnosis/recovery"
    );
    assert_eq!(manifest_path_after, manifest_path);
    assert_eq!(
        fs::read(manifest_path_after).unwrap(),
        manifest_bytes_before,
        "failed startup must not rewrite the selected table manifest"
    );
}
...
#[test]
fn test_partial_unique_index_created_after_checkpoint_skips_soft_deleted_cold_rows() {
    let dir = tempfile::tempdir().unwrap();
    let dsn = format!("file://{}/partial_soft_delete_cold", dir.path().display());
    ...
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (1, 'reuse@example.test', 100)",
        (),
    )
    .unwrap();
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();
    ...
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (2, 'reuse@example.test', NULL)",
        (),
    )
    .unwrap();
    ...
    let duplicate = db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (3, 'reuse@example.test', NULL)",
        (),
    );
    assert!(
        duplicate.is_err(),
        "partial unique must still reject duplicate active hot row"
    );

    let restore = db.execute("UPDATE users SET deleted_at = NULL WHERE id = 1", ());
    assert!(
        restore.is_err(),
        "restoring a soft-deleted cold row must conflict with existing active row"
    );

    assert_eq!(
        query_i64(
            &db,
            "SELECT COUNT(*) FROM users WHERE email = 'reuse@example.test'"
        ),
        2
    );
}
...
#[test]
fn test_partial_unique_index_created_after_checkpoint_blocks_active_cold_rows() {
    let dir = tempfile::tempdir().unwrap();
    let dsn = format!("file://{}/partial_active_cold", dir.path().display());
    ...
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (1, 'active@example.test', NULL)",
        (),
    )
    .unwrap();
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();
    ...
    let duplicate = db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (2, 'active@example.test', NULL)",
        (),
    );
    assert!(
        duplicate.is_err(),
        "partial unique must reject duplicate active row already living in cold volume"
    );

    db.execute("UPDATE users SET deleted_at = 200 WHERE id = 1", ())
        .unwrap();
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (2, 'active@example.test', NULL)",
    ...
    assert_eq!(
        query_i64(
            &db,
            "SELECT COUNT(*) FROM users WHERE email = 'active@example.test' AND deleted_at IS NULL"
        ),
        1
    );
}
...
#[test]
fn test_create_partial_unique_index_after_checkpoint_rejects_matching_cold_duplicates() {
    let dir = tempfile::tempdir().unwrap();
    let dsn = format!("file://{}/partial_duplicate_cold", dir.path().display());
    ...
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (1, 'dup@example.test', NULL)",
        (),
    )
    .unwrap();
    db.execute(
        "INSERT INTO users (id, email, deleted_at)
         VALUES (2, 'dup@example.test', NULL)",
        (),
    )
    .unwrap();
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();
    ...
    assert!(
        create.is_err(),
        "CREATE UNIQUE INDEX ... WHERE must reject duplicate matching cold rows"
    );
}
