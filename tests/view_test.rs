...
fn test_create_view_if_not_exists() {
    ...
    db.execute(
        "CREATE VIEW IF NOT EXISTS my_view AS SELECT * FROM employees",
        (),
    )
    .expect("CREATE VIEW IF NOT EXISTS should not fail");

    // A different definition is drift, not an idempotent replay.
    let drift = db.execute(
        "CREATE VIEW IF NOT EXISTS my_view AS SELECT id FROM employees",
        (),
    );
    assert!(drift.is_err(), "IF NOT EXISTS must reject definition drift");
...
#[test]
fn nested_aggregate_view_exposes_public_unqualified_column_names() {
    let db =
        Database::open("memory://nested_aggregate_view_labels").expect("Failed to create database");
    db.execute(
        "CREATE TABLE view_accounts (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
        (),
    )
    .unwrap();
    db.execute(
    ...
    .unwrap();
    db.execute(
        "INSERT INTO view_accounts VALUES (1, 'cash'), (2, 'bank')",
        (),
    )
    .unwrap();
    db.execute("INSERT INTO view_entries VALUES (1, 1, 10), (2, 1, 20)", ())
        .unwrap();
    db.execute(
        "CREATE VIEW aggregate_accounts AS
         SELECT a.id, a.name, SUM(e.amount) AS total
         FROM view_accounts a
         LEFT JOIN view_entries e ON e.account_id = a.id
         GROUP BY a.id, a.name",
        (),
    )
    .unwrap();
    db.execute(
        "CREATE VIEW positive_aggregate_accounts AS
         SELECT id, name, total FROM aggregate_accounts WHERE COALESCE(total, 0) > 0",
        (),
    )
    .unwrap();
    ...
    let mut result = db
        .query(
            "SELECT id, name, total FROM positive_aggregate_accounts ORDER BY id",
            (),
        )
        .unwrap();
    assert_eq!(result.columns(), ["id", "name", "total"]);
    let row = result.next().unwrap().unwrap();
    assert_eq!(row.get::<i64>(0).unwrap(), 1);
    assert_eq!(row.get::<String>(1).unwrap(), "cash");
