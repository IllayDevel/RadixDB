...
const R2_L06_CRASH_CHILD: &str = "RADIXDB_R2_L06_CRASH_CHILD";
const PROCESS_LOCK_FILE: &str = "LOCK";

#[test]
fn r2_l06_batch_a_abrupt_process_death_recovers_durable_prefix() {
    if let Ok(root) = std::env::var(R2_L06_CRASH_CHILD) {
        let db_path = PathBuf::from(root);
        let ready = db_path.with_extension("ready");
        let dsn = format!(
            "file://{}?checkpoint_on_close=off&sync_mode=full",
            db_path.display()
        );
        let db = Database::open(&dsn).expect("crash child opens database");
        db.execute(
            "CREATE TABLE crash_prefix (id INTEGER PRIMARY KEY, value INTEGER)",
            (),
        )
        .expect("crash child creates table");
        for id in 1..=32i64 {
            db.execute(
                &format!("INSERT INTO crash_prefix VALUES ({id}, {})", id * 10),
                (),
            )
            .expect("crash child commits durable row");
        }
    ...
    let db_path = dir.path().join("abrupt");
    let ready = db_path.with_extension("ready");
    let current_exe = std::env::current_exe().expect("resolve crash-soak test binary");
    let mut child = Command::new(current_exe)
        .arg("--exact")
        .arg("r2_l06_batch_a_abrupt_process_death_recovers_durable_prefix")
        .arg("--nocapture")
        .env(R2_L06_CRASH_CHILD, &db_path)
        .spawn()
    ...
    while !ready.exists() && Instant::now() < deadline {
        if let Some(status) = child.try_wait().expect("poll crash child") {
            panic!("crash child exited before kill barrier: {status}");
        }
        thread::sleep(Duration::from_millis(20));
    }
    assert!(
        ready.exists(),
        "crash child did not reach the durable kill barrier"
    );
    child.kill().expect("abruptly terminate crash child");
    let status = child.wait().expect("reap crash child");
    assert!(!status.success(), "crash child must not close gracefully");

    assert!(
        db_path.join(PROCESS_LOCK_FILE).exists(),
        "abrupt child must leave its process lock inode"
    );
    let db = Database::open(&format!("file://{}", db_path.display()))
        .expect("restart after abrupt process death");
    let count: i64 = db
        .query_one("SELECT COUNT(*) FROM crash_prefix", ())
        .expect("read recovered durable prefix");
    let sum: i64 = db
        .query_one("SELECT SUM(value) FROM crash_prefix", ())
        .expect("verify recovered durable values");
    assert_eq!(count, 32);
    assert_eq!(sum, 5_280);
}
...
/// Legacy truncation fuzz cases intentionally isolate WAL corruption from lock
/// recovery. The real child-process crash oracle above preserves `LOCK`.
fn remove_lock_file(db_path: &Path) {
    let _ = fs::remove_file(db_path.join(PROCESS_LOCK_FILE));
}
    ...
    match rows_result {
        Ok(mut rows) => {
            ...
            while rows.advance() {
                let row = rows
                    .current_row()
                    .expect("advanced crash-soak cursor must expose its current row");
                let value = match row.get(1) {
                    Some(radixdb::Value::Integer(v)) => *v,
                    _ => continue,
                };
