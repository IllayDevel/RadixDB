...
fn test_unique_constraint_survives_snapshot_reopen() {
    ...
    let dsn = format!(
        "file://{}?sync_mode=normal&checkpoint_interval=300&keep_snapshots=5",
        dir.path().display()
    );
...
fn test_view_survives_snapshot_reopen() {
...

    {
        ...
        let views: Vec<String> = db
            .query("SHOW VIEWS", ())
            .unwrap()
            .map(|row| row.unwrap().get_by_name::<String>("view_name").unwrap())
            .collect();
        assert_eq!(views, vec!["active_users"]);

        let create_view: Vec<String> = db
            .query("SHOW CREATE VIEW active_users", ())
            .unwrap()
            .map(|row| row.unwrap().get_by_name::<String>("Create View").unwrap())
            .collect();
        assert_eq!(create_view.len(), 1);
        assert!(create_view[0].contains("SELECT id, name FROM users"));

        // Catalog definition and execution must both survive reopen.
        let names: Vec<String> = db
            .query("SELECT name FROM active_users ORDER BY id", ())
            .unwrap()
            .map(|r| r.unwrap().get::<String>(0).unwrap())
...
#[test]
fn r8_l01_batch_b_expression_default_reopen_uses_clock_transition() {
    let dir = tempfile::tempdir().unwrap();
    let dsn = format!(
        "file://{}?sync_mode=normal&checkpoint_interval=300&keep_snapshots=5",
        dir.path().display()
    ...
    // Wait for an observable clock transition instead of assuming a fixed
    // scheduler delay. If the default is re-evaluated after reopen, it must now
    // differ from the stored value.
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(3);
    loop {
        let now: String = Database::open_in_memory()
            .unwrap()
        ...
        assert!(
            ...
            "CURRENT_TIMESTAMP did not advance before reopen oracle deadline"
        );
