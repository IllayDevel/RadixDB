...
fn test_copy_json_with_specific_columns() {
    ...
    let tmp = tempfile::NamedTempFile::new().unwrap();
    writeln!(tmp.as_file(), r#"{{"id": 1, "name": "Widget"}}"#).unwrap();
...
#[test]
fn r6_copy_json_rejects_unknown_keys_before_mutation() {
    let db = Database::open("memory://json_unknown_key").expect("database");
    db.execute(
        "CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT, price FLOAT DEFAULT 9.99)",
        (),
    )
    ...
    for (columns, payload) in [
        ("", r#"{"id": 1, "name": "one", "unknown": true}"#),
        ("(id, name)", r#"{"id": 2, "name": "two", "price": 1.0}"#),
    ] {
        let mut tmp = tempfile::NamedTempFile::new().unwrap();
        writeln!(tmp.as_file_mut(), "{payload}").unwrap();
        let sql = format!(
            "COPY items {columns} FROM '{}' WITH (FORMAT JSON)",
            tmp.path().display()
        );
        let error = db.execute(&sql, ()).expect_err("unknown key must fail");
        assert!(error.to_string().to_lowercase().contains("column"));
    }

    let count: i64 = db.query_one("SELECT COUNT(*) FROM items", ()).unwrap();
    assert_eq!(count, 0, "unknown keys must not publish a partial row");
}
...
#[test]
fn r2_l06_batch_c_copy_error_rolls_back_the_whole_statement() {
    // A malformed tail must not expose a durable prefix from the same SQL
    // statement, even after the historical 65,536-row boundary.
    let db = Database::open("memory://csv_chunked_partial_failure").unwrap();
    db.execute(
        "CREATE TABLE bench (id INTEGER PRIMARY KEY, val INTEGER)",
        (),
    )
    ...
    writeln!(tmp.as_file(), "id,val").unwrap();
    for id in 0..65_536 {
        writeln!(tmp.as_file(), "{id},{}", id * 2).unwrap();
    }
    writeln!(tmp.as_file(), "not-an-integer,0").unwrap();

    let sql = format!(
        "COPY bench FROM '{}' WITH (FORMAT CSV, HEADER true)",
        tmp.path().display()
    );
    let error = db.execute(&sql, ()).expect_err("malformed tail must fail");
...
#[test]
fn copy_memory_budget_fails_closed_without_a_durable_prefix() {
    let directory = tempfile::tempdir().unwrap();
    let dsn = format!(
        "file://{}?copy_max_transaction_bytes=1024",
        directory.path().join("copy-limit").display()
    );
    ...
    let error = db
        .execute(&sql, ())
        .expect_err("COPY must hit its memory budget");
    assert!(
        error
            .to_string()
            .contains("COPY transaction memory budget exceeded"),
    ...
    let count: i64 = reopened
        .query_one("SELECT COUNT(*) FROM bench", ())
        .unwrap();
    assert_eq!(count, 0);
}
...
#[test]
fn test_copy_json_large_statement_commits_as_one_unit() {
    let db = Database::open("memory://json_chunked_success").unwrap();
    db.execute(
        "CREATE TABLE bench (id INTEGER PRIMARY KEY, val INTEGER)",
        (),
    )
    ...
    let tmp = tempfile::NamedTempFile::new().unwrap();
    for id in 0..65_537 {
        writeln!(tmp.as_file(), r#"{{"id": {id}, "val": {}}}"#, id * 3).unwrap();
    }
    ...
    assert_eq!(db.execute(&sql, ()).unwrap(), 65_537);
...
fn test_copy_json_case_insensitive_unicode_keys_with_columns() {
    ...
    // (radixdb lowercases column names, so "name" is stored as "name")
