...
#[test]
fn r8_l01_batch_a_as_of_timestamp_precision_is_asserted() {
    ...
    let timestamps = [
        "2050-01-01 12:00:00",        // Date and time
        "2050-01-01 12:00:00.123",    // With milliseconds
        "2050-01-01 12:00:00.123456", // With microseconds
    ];
    ...
    for ts in timestamps {
        let rows = db
            ...
            .unwrap_or_else(|error| panic!("timestamp format '{ts}' must execute: {error}"));
        assert_eq!(rows.count(), 1, "timestamp format '{ts}' lost the row");
    }
}
...
fn test_as_of_select_specific_columns() {
    ...
    let mut result = db
        .query("SELECT a, c FROM multi_col AS OF TRANSACTION 2", ())
        .expect("AS OF with specific columns should execute");
    assert_eq!(result.columns(), &["a", "c"]);
    let row = result.next().expect("one temporal row").expect("valid row");
    assert_eq!(row.get::<i64>(0).unwrap(), 1);
    let expected_decimal = 314.0 / 100.0;
    assert!((row.get::<f64>(1).unwrap() - expected_decimal).abs() < f64::EPSILON);
    ...
    let mut result = db
        ...
        .expect("AS OF with specific columns and WHERE should execute");
    assert_eq!(result.columns(), &["b", "d"]);
    let row = result.next().expect("one filtered row").expect("valid row");
