...
fn test_not_with_and_condition() {
    ...
    // Exclude Laptop and Smartphone; the all-NULL row remains UNKNOWN after
    // NOT and is therefore not admitted by WHERE.
    assert_eq!(
        count, 7,
        "Expected 7 products not (Electronics AND expensive), excluding UNKNOWN"
    );
}
...
fn test_not_with_composite_condition() {
    ...
    // The all-NULL row remains UNKNOWN after NOT and is excluded by WHERE.
    assert_eq!(
        count, 7,
        "Expected 7 products not (expensive Electronics in stock), excluding UNKNOWN"
    );
}
