...
/// Test the supported RIGHT JOIN contract exactly.
#[test]
fn test_right_join() {
    ...
    let rows = db
        ...
        .expect("RIGHT JOIN is a supported relational operator");
    let mut actual = rows
        .map(|row| {
            let row = row.expect("RIGHT JOIN row");
            (
    ...
    assert_eq!(
        actual,
        vec![
            ("Alice Smith".to_string(), Some(1001)),
            ("Alice Smith".to_string(), Some(1003)),
            ("Bob Johnson".to_string(), Some(1002)),
            ("Charlie Brown".to_string(), Some(1004)),
            ("Diana Adams".to_string(), None),
        ]
    );
}
