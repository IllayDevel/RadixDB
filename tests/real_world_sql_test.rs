...
use radixdb::parser::{parse_sql, Statement};
...
fn test_create_partial_unique_index() {
    let stmt =
        parse_ok("CREATE UNIQUE INDEX idx_email_live ON employees(email) WHERE deleted_at IS NULL");
    match stmt {
        Statement::CreateIndex(index) => {
            assert!(index.is_unique);
            assert_eq!(index.index_name.value(), "idx_email_live");
            assert_eq!(index.table_name.value(), "employees");
            assert_eq!(index.columns.len(), 1);
            assert!(index.where_clause.is_some());
            assert_eq!(
                index.to_string(),
                "CREATE UNIQUE INDEX idx_email_live ON employees (email) WHERE (deleted_at IS NULL)"
            );
        }
...
#[test]
fn test_create_partial_index_after_using_and_with() {
    let stmt = parse_ok(
        "CREATE INDEX idx_email_active ON employees(email)
    ...
    match stmt {
        Statement::CreateIndex(index) => {
            assert!(!index.is_unique);
            assert!(index.index_method.is_some());
            assert_eq!(index.options.len(), 1);
            assert_eq!(index.options[0].0, "fillfactor");
            assert_eq!(index.options[0].1.to_string(), "90");
            assert!(index.where_clause.is_some());
            assert_eq!(
                index.to_string(),
                "CREATE INDEX idx_email_active ON employees (email) USING BTREE WITH (fillfactor = 90) WHERE (active = TRUE)"
            );
        }
        _ => panic!("expected CreateIndex statement"),
    }
