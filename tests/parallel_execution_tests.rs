...
/// EXPLAIN ANALYZE reports only request-owned facts when the runtime operator
...
fn test_explain_analyze_shows_parallel() {
    ...
    let plan = explain_output.join("\n");
    assert!(plan.contains("SELECT (actual time=") && plan.contains("rows=7350)"));
    assert!(plan.contains("-> Seq Scan on explain_parallel_test"));
    assert!(!plan.contains("Parallel Seq Scan"));
}
...
fn test_parallel_error_handling() {
    ...
    // Division by zero evaluates to a non-matching predicate row under the
    // current SQL arithmetic contract; every other row matches.
    let count: i64 = db
        .query_one(
            "SELECT COUNT(*) FROM error_test WHERE value / (id - 5000) > 0",
            (),
        )
        .expect("parallel arithmetic predicate must complete");
    assert_eq!(count, 9_999);

    // Non-coercible equality is a deterministic non-match, not an arbitrary
    // success/error branch.
    let rows = db
        .query("SELECT * FROM error_test WHERE value = 'not_a_number'", ())
        .expect("non-coercible comparison must execute as a non-match");
