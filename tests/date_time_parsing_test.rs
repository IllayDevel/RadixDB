...
#[test]
fn r8_l01_batch_a_extract_capability_is_asserted() {
    let sql = "SELECT id, EXTRACT(YEAR FROM created_at) as year FROM events";
    let statement = parse_sql(sql).expect("EXTRACT must parse as a supported capability");
    assert_eq!(
        statement.len(),
        1,
        "EXTRACT query must produce one statement"
    );
}
