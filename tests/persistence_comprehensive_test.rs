...
#[test]
fn test_partial_unique_index_persistence() {
    let dir = tempdir().unwrap();
    let db_path = dir.path().join("test.db");
    let dsn = format!("file://{}", db_path.display());

    {
        ...
        .unwrap();
        db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (1, 'a@example.test', NULL)",
            (),
        )
        .unwrap();

        let duplicate = db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (2, 'a@example.test', NULL)",
            (),
        );
        assert!(
            duplicate.is_err(),
            "duplicate active row must fail before reopen"
        );
...

    {
        ...
        let duplicate = db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (3, 'a@example.test', NULL)",
            (),
        );
        assert!(
            duplicate.is_err(),
            "duplicate active row must fail after reopen"
        );

        db.execute("UPDATE users SET deleted_at = 1 WHERE id = 1", ())
            .unwrap();
        db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (3, 'a@example.test', NULL)",
            (),
        ...
        let restore_duplicate = db.execute("UPDATE users SET deleted_at = NULL WHERE id = 1", ());
        assert!(
            restore_duplicate.is_err(),
            "restoring soft-deleted row to active must re-check partial unique index after reopen"
        );
...
#[test]
fn test_partial_unique_index_wal_replay_without_checkpoint() {
    let dir = tempdir().unwrap();
    let db_path = dir.path().join("test.db");
    let dsn = format!(
...

    {
        ...
        .unwrap();
        db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (1, 'wal@example.test', NULL)",
            (),
        )
        .unwrap();
        db.close().unwrap();
...

    {
        let db = Database::open(&dsn).unwrap();
        let duplicate = db.execute(
            "INSERT INTO users (id, email, deleted_at) VALUES (2, 'wal@example.test', NULL)",
            (),
        );
        assert!(
            duplicate.is_err(),
            "partial unique predicate must survive WAL replay without checkpoint"
        );
