...
fn test_query_cache_basic() {
    ...
    let db = Database::open("memory://query-cache-basic").expect("Failed to create database");
...
fn test_query_cache_multiple_tables() {
    let db =
        Database::open("memory://query-cache-multiple-tables").expect("Failed to create database");
...
fn test_query_cache_where_clauses() {
    let db = Database::open("memory://query-cache-where").expect("Failed to create database");
...
fn test_query_cache_aggregations() {
    let db =
        Database::open("memory://query-cache-aggregations").expect("Failed to create database");
...
fn test_query_cache_with_dml() {
    let db = Database::open("memory://query-cache-dml").expect("Failed to create database");
...
/// Different harmless formatting remains executable with exact-source keys.
#[test]
fn test_query_cache_distinct_source_identity() {
    let db =
        Database::open("memory://query-cache-source-identity").expect("Failed to create database");

    db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY)", ())
        .unwrap();
    db.execute("INSERT INTO test VALUES (1)", ()).unwrap();

    // These are semantically equal, but cache identity deliberately preserves bytes.
    let queries = vec![
        "SELECT * FROM test",
        "SELECT  *  FROM  test",
        "SELECT * FROM test ",
