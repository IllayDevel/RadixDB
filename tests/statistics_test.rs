...
/// Targeted ANALYZE must surface a missing target.
#[test]
fn test_analyze_nonexistent_table() {
    let db = Database::open_in_memory().expect("Failed to create database");

    let error = match db.query("ANALYZE nonexistent_table", ()) {
        Err(error) => error,
        Ok(_) => panic!("targeted ANALYZE must not report false success"),
    };
    assert!(error.to_string().contains("nonexistent_table"), "{error}");
}
...
fn test_selectivity_estimator() {
    use radixdb::storage::SelectivityEstimator;
