...
fn test_json_keys_non_object() {
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_KEYS on array should return NULL"
    );
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_KEYS on string should return NULL"
    );
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_KEYS on number should return NULL"
    );
}
...
fn test_json_functions_with_null() {
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_TYPE(NULL) should return NULL"
    );
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_VALID(NULL) should return NULL"
    );
    ...
    assert!(
        row.is_null(0).unwrap(),
        "JSON_KEYS(NULL) should return NULL"
    );
}
...
fn test_json_functions_with_table() {
    ...
    let id: i64 = row.get(0).unwrap();
    assert_eq!(id, 5);
    assert!(row.is_null(1).unwrap(), "JSON_TYPE of NULL should be NULL");
}
