...
// =============================================================================
// Parameterized DECIMAL is accepted now that precision/scale are represented and enforced.
// =============================================================================
#[test]
fn test_bugs2_20_decimal_syntax() {
    ...
    .expect("parameterized DECIMAL should be accepted");
    db.execute(
        "INSERT INTO t VALUES (1, CAST('123.45' AS DECIMAL(10, 2)))",
        (),
    )
    .expect("a value inside DECIMAL(10,2) should be accepted");

    let create_sql: String = db
        .query("SHOW CREATE TABLE t", ())
        .expect("SHOW CREATE TABLE should expose DECIMAL parameters")
        .next()
        .expect("SHOW CREATE TABLE row")
        .expect("SHOW CREATE TABLE result")
        .get(1)
        .expect("CREATE SQL column");
    assert!(create_sql.contains("DECIMAL(10,2)"), "{create_sql}");
}
