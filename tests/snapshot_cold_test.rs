...
/// Verify that seal defers during active snapshot transactions.
/// Both the snapshot and auto-commit queries must retain their logical views
/// while row-level MVCC provenance prevents physical publication.
#[test]
fn test_seal_defers_during_snapshot() {
    let dir = tempfile::tempdir().unwrap();
    let dsn = format!("file://{}", dir.path().to_str().unwrap());
    let db = Database::open(&dsn).unwrap();
    ...
    // Seal defers while the snapshot is active. A public checkpoint may not
    // claim a complete cycle while protected hot rows prevent WAL truncation.
    let checkpoint_error = db.execute("PRAGMA CHECKPOINT", ()).unwrap_err();
    assert!(
        checkpoint_error.to_string().contains("hot rows unsealed"),
        "unexpected incomplete checkpoint outcome: {checkpoint_error}"
    );
...
fn test_snapshot_update_cold_then_reseal() {
    ...
    // The post-snapshot update must stay hot so the active snapshot can keep
    // observing the cold pre-update version. The checkpoint therefore reports
    // an incomplete cycle instead of falsely claiming durable completion.
    let checkpoint_error = db.execute("PRAGMA CHECKPOINT", ()).unwrap_err();
    assert!(
        checkpoint_error.to_string().contains("hot rows unsealed"),
        "unexpected incomplete checkpoint outcome: {checkpoint_error}"
