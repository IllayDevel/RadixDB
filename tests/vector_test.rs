...
use radixdb::storage::v6::{open_snapshot_manifest, SNAPSHOT_MANIFEST_FILE};
use radixdb::Database;

/// Collect query results, unwrapping each row.
fn collect_rows(result: radixdb::Rows) -> Vec<radixdb::ResultRow> {
...
#[test]
fn r6_hnsw_rejects_zero_search_and_construction_budgets() {
    let db = Database::open("memory://hnsw_zero_budget").expect("database");
    db.execute(
        "CREATE TABLE vecs (id INTEGER PRIMARY KEY, v VECTOR(4))",
        (),
    )
    ...
    for option in ["ef_construction = 0", "ef_search = 0"] {
        let sql = format!("CREATE INDEX idx_zero ON vecs(v) USING HNSW WITH ({option})");
        let error = db
            .execute(&sql, ())
            .expect_err("zero HNSW budget must fail");
...
fn test_hnsw_graph_serialization() {
    use radixdb::storage::index::{HnswDistanceMetric, HnswIndex};
    use radixdb::Index;
    ...
    // Add some vectors
    let v1 = radixdb::Value::vector(vec![1.0f32, 0.0, 0.0]);
    let v2 = radixdb::Value::vector(vec![0.0f32, 1.0, 0.0]);
    let v3 = radixdb::Value::vector(vec![0.5f32, 0.5, 0.0]);
...
// ---------------------------------------------------------------------------
// HNSW Recovery from Canonical Snapshot Rows
// ---------------------------------------------------------------------------

#[test]
fn r8_l01_batch_c_hnsw_graph_snapshot_recovery() {
    let dir = tempfile::tempdir().expect("tmpdir");
    let db_path = format!(
    ...
    // Phase 1: Create DB, insert data, create HNSW index, snapshot
    {
        ...
        db.execute("PRAGMA SNAPSHOT", ())
            .expect("snapshot must persist canonical vector rows");
    }

    let snapshot_root = dir.path().join("hnsw_snap/snapshots");
    let snapshots = std::fs::read_dir(&snapshot_root)
        .expect("snapshot root")
        .filter_map(Result::ok)
        .filter(|entry| entry.path().join(SNAPSHOT_MANIFEST_FILE).is_file())
        .collect::<Vec<_>>();
    assert_eq!(snapshots.len(), 1, "exactly one snapshot must be committed");
    let snapshot = &snapshots[0];
    let manifest = open_snapshot_manifest(snapshot.path()).expect("open snapshot manifest");
    assert_eq!(
        snapshot.file_name().to_string_lossy(),
    ...
    assert!(
        manifest
            .members()
            .iter()
            .all(|member| !member.relative_path().to_string_lossy().ends_with(".bin")),
        "snapshot must not persist a second HNSW truth beside canonical rows"
    );

    // Phase 2: Reopen DB — recovery rebuilds the HNSW graph from snapshot rows.
    {
        let db = Database::open(&db_path).expect("reopen db");
...
// ---------------------------------------------------------------------------
// UNIQUE HNSW contract
// ---------------------------------------------------------------------------

#[test]
fn test_hnsw_unique_is_rejected_without_partial_index_publication() {
    let db = Database::open("memory://hnsw_unique_rejected").expect("db");
    db.execute(
        "CREATE TABLE vecs (id INTEGER PRIMARY KEY, v VECTOR(3))",
        (),
    )
    ...
    let error = db
        ...
        .expect_err("UNIQUE HNSW definition must be rejected");
    let message = error.to_string();
    assert!(
        message.contains("HNSW") && message.contains("UNIQUE"),
        "error must identify the unsupported UNIQUE HNSW contract: {message}"
    );

    db.execute(
        "CREATE INDEX idx_v ON vecs(v) USING HNSW WITH (metric = 'cosine')",
        (),
    )
    .expect("failed UNIQUE DDL must not reserve the index name");
    db.execute("INSERT INTO vecs VALUES (1, '[0.0, 0.0, 0.0]')", ())
        .expect("insert first zero vector");
    db.execute("INSERT INTO vecs VALUES (2, '[0.0, 0.0, 0.0]')", ())
        .expect("ordinary HNSW index must not own UNIQUE semantics");
...
fn test_explain_vector_hnsw_search() {
    ...
    assert!(
        ...
        "Expected exact Vector Scan, got:\n{}",
        plan
    );
    assert!(
        !plan.contains("HNSW"),
        "ordinary SQL must not claim ANN semantics:\n{}",
        plan
    );
    assert!(
...
fn test_explain_analyze_vector_hnsw() {
    ...
    assert!(
        plan.contains("SELECT (actual time=") && plan.contains("rows=2)"),
...
fn test_explain_vector_cosine() {
    ...
    assert!(
        plan.contains("Vector Scan"),
        "Expected exact cosine scan, got:\n{}",
        plan
    );
    assert!(!plan.contains("HNSW"));
    assert!(
...
fn test_explain_vector_with_where() {
    ...
    assert!(
        plan.contains("Vector Scan"),
        "Expected exact filtered scan, got:\n{}",
        plan
    );
    assert!(!plan.contains("HNSW"));
    assert!(
