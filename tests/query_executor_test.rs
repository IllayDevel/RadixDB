...
#[test]
fn test_streaming_simple_projection_reorder_alias_and_duplicate() {
    let db =
        Database::open("memory://streaming_simple_projection").expect("Failed to create database");
    db.execute(
        "CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, payload TEXT)",
        (),
    )
    .expect("Failed to create table");
    db.execute("INSERT INTO t VALUES (1, 'Alice', 'ignored payload')", ())
        .expect("Failed to insert row");
    db.execute("INSERT INTO t VALUES (2, 'Bob', 'ignored payload')", ())
        .expect("Failed to insert row");

    let result = db
        .query("SELECT name AS label, id FROM t", ())
        .expect("Failed to query projected columns");
    let mut rows = Vec::new();
    for row in result {
        let row = row.expect("Failed to get row");
        assert_eq!(row.len(), 2);
        rows.push((row.get::<i64>(1).unwrap(), row.get::<String>(0).unwrap()));
    }
    rows.sort_by_key(|(id, _)| *id);
    assert_eq!(rows, vec![(1, "Alice".to_string()), (2, "Bob".to_string())]);

    // Duplicate projection is part of the scanner projection contract.
    let result = db
        .query("SELECT id, id FROM t", ())
        .expect("Failed to query duplicate projection");
    let mut pairs = Vec::new();
    for row in result {
        let row = row.expect("Failed to get row");
        assert_eq!(row.len(), 2);
        pairs.push((row.get::<i64>(0).unwrap(), row.get::<i64>(1).unwrap()));
    }
    pairs.sort_unstable();
