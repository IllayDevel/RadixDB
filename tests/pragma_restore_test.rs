...
/// Verify latest selection after multiple committed snapshots.
#[test]
fn test_restore_latest_after_multiple_snapshots() {
    let dir = tempfile::tempdir().unwrap();
    ...
    // Keep creation order unambiguous.
...
/// Verify PRAGMA RESTORE replaces the physical generation without creating
/// any legacy volume tree.
#[test]
fn test_restore_replaces_physical_generation_without_legacy_volumes() {
    let dir = tempfile::tempdir().unwrap();
    ...
    // Snapshot then checkpoint to create a DATA artifact in the live branch.
    db.execute("PRAGMA SNAPSHOT", ()).unwrap();
    db.execute("PRAGMA CHECKPOINT", ()).unwrap();

    assert!(db_dir.join("artifacts").join("data").exists());
    assert!(!db_dir.join("volumes").exists());
    ...
    // Restore the snapshotted WAL/catalog/artifact generation.
    db.execute("PRAGMA RESTORE", ()).unwrap();
    assert_eq!(query_i64(&db, "SELECT COUNT(*) FROM t"), 100);

    assert!(db_dir.join("artifacts").exists());
    assert!(db_dir.join("manifests").exists());
    assert!(!db_dir.join("volumes").exists());
...
fn test_pragma_restore_indexes_and_views() {
    ...
    let direct_join_count = query_i64(
        &db,
        "SELECT COUNT(*) FROM users u JOIN orders o ON u.id = o.user_id",
    );
    assert_eq!(
        direct_join_count, 3,
        "restored cold tables must still join correctly"
    );
...
fn snapshot_fingerprint(root: &std::path::Path) -> Vec<(std::path::PathBuf, Vec<u8>)> {
    fn walk(
        root: &std::path::Path,
...
#[cfg(feature = "test-failpoints")]
#[test]
fn physical_restore_swap_failures_roll_back_without_mutating_source() {
    use std::sync::atomic::Ordering;

    let _guard = radixdb::test_failpoints::FailpointGuard::new();
    let directory = tempfile::tempdir().unwrap();
    let database_root = directory.path().join("restore_failpoints");
    let dsn = format!("file://{}", database_root.display());
    let db = Database::open(&dsn).unwrap();
    db.execute(
    ...
    .unwrap();
    db.execute("INSERT INTO items VALUES (1, 'snapshot')", ())
        .unwrap();
    db.execute("PRAGMA SNAPSHOT", ()).unwrap();

    let snapshot_root = database_root.join("snapshots");
    let source = snapshot_fingerprint(&snapshot_root);
    db.execute("INSERT INTO items VALUES (2, 'live')", ())
        .unwrap();

    for failpoint in [
        &radixdb::test_failpoints::RESTORE_FAIL_AFTER_OLD_MOVE,
        &radixdb::test_failpoints::RESTORE_FAIL_AFTER_NEW_MOVE,
    ] {
        failpoint.store(true, Ordering::Release);
        let error = db.restore_snapshot(None).unwrap_err();
        failpoint.store(false, Ordering::Release);
        assert!(
            error
                .to_string()
        ...
        assert_eq!(query_i64(&db, "SELECT COUNT(*) FROM items"), 2);
        assert_eq!(snapshot_fingerprint(&snapshot_root), source);
        for transient in [
            ".physical-restore-state",
            ".physical-restore-staging",
            ".physical-restore-backup",
        ] {
            assert!(!database_root.join(transient).exists());
        }
    }

    db.restore_snapshot(None).unwrap();
    assert_eq!(query_i64(&db, "SELECT COUNT(*) FROM items"), 1);
    assert_eq!(snapshot_fingerprint(&snapshot_root), source);
    db.close().unwrap();

    let reopened = Database::open(&dsn).unwrap();
    assert_eq!(query_i64(&reopened, "SELECT COUNT(*) FROM items"), 1);
