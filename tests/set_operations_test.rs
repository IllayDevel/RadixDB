...
#[test]
fn compound_tail_does_not_leak_or_reorder_left_operand_dependencies() {
    let db = create_test_db("union_compound_tail");
    db.execute(
        "CREATE TABLE values_by_id (id INTEGER PRIMARY KEY, payload INTEGER); \
         INSERT INTO values_by_id VALUES (1, 30), (2, 10)",
        (),
    )
    .unwrap();
    ...
    let values: Vec<i64> = db
        .query(
            "SELECT payload FROM values_by_id \
             UNION SELECT 20 ORDER BY 1 LIMIT 2",
            (),
        )
        .unwrap()
        .map(|row| row.and_then(|row| row.get(0)))
        .collect::<radixdb::Result<_>>()
        .unwrap();
