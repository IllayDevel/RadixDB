...
#[test]
fn test_savepoint_rollback_releases_uuid_cold_delete_claim() {
    let dir = tempfile::tempdir().expect("failed to create temp dir");
    let dsn = format!("file://{}/uuid_savepoint_claim", dir.path().display());
    let db1 = Database::open(&dsn).expect("failed to open database");
    db1.execute(
        "CREATE TABLE t (id UUID PRIMARY KEY, val TEXT NOT NULL)",
        (),
    )
    .unwrap();
    db1.execute(
        "INSERT INTO t VALUES ('018f8f30-7b5c-7000-8000-000000000001', 'before')",
        (),
    )
    .unwrap();
    db1.execute("PRAGMA CHECKPOINT", ()).unwrap();

    let db2 = db1.clone();
    db1.execute("BEGIN", ()).unwrap();
    db1.execute("SAVEPOINT before_delete", ()).unwrap();
    db1.execute(
        "DELETE FROM t WHERE id = '018f8f30-7b5c-7000-8000-000000000001'",
        (),
    )
    .unwrap();
    db1.execute("ROLLBACK TO SAVEPOINT before_delete", ())
        .unwrap();

    db2.execute("BEGIN", ()).unwrap();
    db2.execute(
        "UPDATE t SET val = 'after' WHERE id = '018f8f30-7b5c-7000-8000-000000000001'",
        (),
    )
    .expect("savepoint rollback must release the UUID cold-row claim");
    db2.execute("COMMIT", ()).unwrap();
    db1.execute("COMMIT", ()).unwrap();

    let value: String = db1
        .query_one(
            "SELECT val FROM t WHERE id = '018f8f30-7b5c-7000-8000-000000000001'",
            (),
        )
        .unwrap();
...
fn tx_q_str(tx: &mut radixdb::api::transaction::Transaction, sql: &str) -> Option<String> {
...
fn tx_q_i64(tx: &mut radixdb::api::transaction::Transaction, sql: &str) -> i64 {
...
/// at its start time.
#[test]
fn r3_l01_batch_c_scan_consistency_during_concurrent_mods() {
    ...
    let scanner = thread::spawn(move || {
        ...
        let mut scanned_rows: Vec<(i64, String)> = Vec::new();
        for row_result in result {
            let row = row_result.expect("row iteration failed");
            let id: i64 = row.get(0).expect("get id failed");
            let value: String = row.get(1).expect("get value failed");
            scanned_rows.push((id, value));
        }
        scanned_rows
    });
    ...
    let modifier = thread::spawn(move || {
        ...
        for &id in &[50, 60, 70] {
            db_mod
                ...
                .expect("update committed epoch");
        }
    });

    let scanned_rows = scanner.join().expect("scanner thread panicked");
    modifier.join().expect("modifier thread panicked");

    // A statement may begin at any one of the six auto-commit boundaries,
    // but every row/value must belong to that same whole epoch.
    let mut epoch: Vec<(i64, String)> = (1..=100).map(|id| (id, format!("row_{id}"))).collect();
    let mut allowed_epochs = vec![epoch.clone()];
    for id in [10, 20, 30] {
        epoch.retain(|(row_id, _)| *row_id != id);
        allowed_epochs.push(epoch.clone());
    }
    for id in [50, 60, 70] {
        let (_, value) = epoch
            .iter_mut()
            .find(|(row_id, _)| *row_id == id)
            .expect("updated row exists in epoch model");
        *value = format!("changed_{id}");
        allowed_epochs.push(epoch.clone());
    }
    ...
    assert!(
        allowed_epochs
            .iter()
            .any(|allowed| allowed == &scanned_rows),
        "scan mixed publication epochs: {:?}",
        scanned_rows
    );
    for window in scanned_rows.windows(2) {
        assert!(
