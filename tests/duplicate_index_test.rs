...
fn test_if_not_exists() {
    ...
    // IF NOT EXISTS is scoped to the requested index identity, not to any
    // unrelated index on the same column. A conflicting definition must not
    ...
    assert!(
        result.is_err(),
        "IF NOT EXISTS must not hide a conflicting index definition"
    );

    // Repeating the exact same definition is idempotent.
    db.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_name3 ON duplicate_test3(name)",
        (),
    )
    .expect("exact IF NOT EXISTS definition should succeed");
}
