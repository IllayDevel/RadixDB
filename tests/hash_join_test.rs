...
#[test]
fn test_hash_join_simple_select_uses_projected_output_contract() {
    let db = create_test_db("hash_join_projected_output_contract");

    db.execute(
        "CREATE TABLE hj_left_proj (id INTEGER PRIMARY KEY, val INTEGER, unused INTEGER)",
        (),
    )
    .unwrap();
    db.execute(
        "CREATE TABLE hj_right_proj (id INTEGER PRIMARY KEY, data INTEGER, unused INTEGER)",
        (),
    )
    .unwrap();

    db.execute(
        "INSERT INTO hj_left_proj VALUES (2, 20, 2000), (1, 10, 1000), (3, 30, 3000)",
        (),
    )
    .unwrap();
    db.execute(
        "INSERT INTO hj_right_proj VALUES (1, 100, 9000), (3, 300, 7000)",
        (),
    )
    .unwrap();
    ...
    let result = db
        .query(
            "SELECT l.val, r.data \
             FROM hj_left_proj l INNER JOIN hj_right_proj r ON l.id = r.id",
            (),
        )
        .unwrap();

    assert_eq!(result.column_count(), 2);
    assert_eq!(result.columns(), &["val".to_string(), "data".to_string()]);

    let rows: Vec<_> = result.map(|row| row.unwrap()).collect();
    assert_eq!(rows.len(), 2);
    for row in rows {
        let val: i64 = row.get(0).unwrap();
        let data: i64 = row.get(1).unwrap();
        assert!(matches!((val, data), (10, 100) | (30, 300)));
    }
}
