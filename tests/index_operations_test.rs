...
fn test_multi_column_index_query_optimization() {
    ...
    let index = results
        .iter()
        .find(|entry| entry.0 == "idx_sales_region_product_year")
        .expect("SHOW INDEXES includes the named multi-column index");
    assert!(index.1.contains("region"));
    assert!(index.1.contains("product"));
    assert!(index.1.contains("year"));
    assert_eq!(index.2, "MULTICOLUMN");
}
