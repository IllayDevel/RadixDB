...
//!
//! These tests exercise the code paths in crates/radixdb-executor/src/subquery/
...
#[test]
fn test_anti_join_inner_key_projection_preserves_filter_columns() {
    let db = Database::open("memory://anti_join_projected_key").expect("Failed");
    db.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)", ())
        .expect("Failed");
    db.execute(
        "CREATE TABLE logins (
    ...
    db.execute(
        "INSERT INTO logins VALUES (1, 1, 'failed', 'large ignored payload')",
        (),
    )
    .expect("Failed");
    db.execute(
        "INSERT INTO logins VALUES (2, 2, 'ok', 'large ignored payload')",
        (),
    )
    .expect("Failed");
    db.execute(
        "INSERT INTO logins VALUES (3, 3, 'failed', 'large ignored payload')",
        (),
    )
    .expect("Failed");
        ...
        .query(
            "SELECT id FROM users
             WHERE NOT EXISTS (
                 ...
                 WHERE logins.user_id = users.id AND logins.status = 'ok'
             )",
            (),
        )
        .expect("Query failed");
