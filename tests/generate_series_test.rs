...
/// The hard cap is fail-closed: silent truncation changes query results.
#[test]
fn test_generate_series_cap_rejects_implicit_truncation() {
    ...
    let error = match db.query(
        ...
        Err(error) => error,
        Ok(_) => panic!("an oversized series must not be silently truncated"),
    };
    assert!(error
        .to_string()
        .contains("would produce more than 10000000 rows"));
}
