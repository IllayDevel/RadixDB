...
#[test]
fn test_count_join_with_outer_filter_and_pk_inner() {
    let db = Database::open("memory://join_count_pk_inner").expect("Failed to create database");

    db.execute(
        "CREATE TABLE parent_count_join (id INTEGER PRIMARY KEY, name TEXT)",
        (),
    )
    .expect("Failed to create parent table");
    db.execute(
        "CREATE TABLE child_count_join (
    ...
    for id in 1..=10 {
        db.execute(
            &format!("INSERT INTO parent_count_join VALUES ({id}, 'p{id}')"),
            (),
        )
        .expect("Failed to insert parent row");
    }

    for id in 1..=30 {
        let parent_id = (id % 10) + 1;
        let parent_table = if id <= 20 { 59 } else { 58 };
        let bucket = id % 5;
        db.execute(
            &format!(
                "INSERT INTO child_count_join VALUES ({id}, {parent_id}, {parent_table}, {bucket})"
            ),
            (),
        )
        .expect("Failed to insert child row");
    }

    let count: i64 = db
        .query_one(
            "SELECT COUNT(*)
             FROM child_count_join c
             JOIN parent_count_join p ON c.parent_id = p.id
             WHERE c.parent_table = 59 AND c.bucket BETWEEN 1 AND 3",
            (),
        )
        .expect("Failed to execute aggregate join");
