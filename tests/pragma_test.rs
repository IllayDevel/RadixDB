...
/// Unknown PRAGMA names must fail loudly instead of returning an empty
/// successful result. This protects maintenance scripts from silent typos.
#[test]
fn test_unknown_pragma_returns_error() {
    let db = Database::open("memory://pragma_unknown").expect("Failed to create database");

    let err = db
        .execute("PRAGMA chekpoint", ())
        .expect_err("unknown PRAGMA should return an explicit error");
    let message = err.to_string();

    assert!(
        message.contains("Unknown PRAGMA"),
        "expected unknown PRAGMA diagnostic, got: {message}"
    );
    assert!(
        message.contains("chekpoint"),
        "diagnostic should include the mistyped pragma name, got: {message}"
    );
}
