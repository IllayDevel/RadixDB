...
fn test_same_row_updates_across_sessions() {
    ...
    // Session 1: Create and insert (use INTEGER PRIMARY KEY as required by radixdb)
    {
        let db = Database::open(&dsn).unwrap();
        db.execute(
...
/// Inline UNIQUE constraints must remain durable through the catalog transaction.
#[test]
