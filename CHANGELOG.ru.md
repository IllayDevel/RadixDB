...
- Автоматическое освобождение неизменяемых компонентов сериализовано с
  владением physical restore и compaction. Каталог quarantine больше не может
  возникнуть после начала замены физического корня.
- TCP-проверка отмены теперь запускает активную параллельную агрегацию до
  разрыва соединения, а конкурентный reader при переименовании индекса повторяет
  типизированный fail-closed исход `SchemaChanged`.
- File-backed inventory в CI выполняется последовательно и ограничен по времени,
  поэтому зависший checkpoint-сценарий завершается конечной диагностикой, а не
  занимает runner без ограничения.
