...
use radixdb_core::{parse_timestamp, Error, Result, Row, RowVec, Value};
...
pub type TableRowStream = Box<dyn Iterator<Item = Result<(i64, Row)>> + Send>;
...
pub trait TableValuedFunction: Send + Sync {
    ...
    /// Metadata used by the common function-introspection surface.
    ...
    /// Produce rows incrementally. Existing external TVFs remain source
    /// compatible through the materializing default; built-ins override this
    /// method so executor backpressure reaches the producer.
    fn stream(&self, args: &[Value], limit: Option<usize>) -> Result<TableRowStream> {
        Ok(Box::new(self.generate(args, limit)?.into_iter().map(Ok)))
    }
}
...
impl GenerateSeriesFunction {
    fn safety_limit_error() -> Error {
        Error::invalid_argument(format!(
            "GENERATE_SERIES would produce more than {MAX_TVF_ROWS} rows; add a LIMIT not exceeding {MAX_TVF_ROWS}"
        ))
    }

    fn iterative_budget(limit: Option<usize>) -> (usize, bool) {
        match limit {
            Some(limit) if limit <= MAX_TVF_ROWS => (limit, false),
            _ => (MAX_TVF_ROWS, true),
        }
    }
    ...
    fn generate_integer(args: &[Value], limit: Option<usize>) -> Result<RowVec> {
        ...
        let full_count = (stop as i128 - start as i128) / step as i128 + 1;
        debug_assert!(full_count > 0);
        let requested_count = limit.map_or(full_count, |limit| limit as i128);
        if full_count > MAX_TVF_ROWS as i128 && requested_count > MAX_TVF_ROWS as i128 {
            return Err(Self::safety_limit_error());
        }
        let max_rows = usize::try_from(full_count.min(requested_count))
            .map_err(|_| Self::safety_limit_error())?;
    ...
    fn generate_timestamp(args: &[Value], limit: Option<usize>) -> Result<RowVec> {
        ...
        let (max_rows, error_on_exhaustion) = Self::iterative_budget(limit);
    ...
    fn stream_integer(args: &[Value], limit: Option<usize>) -> Result<TableRowStream> {
        let start = Self::to_i64(&args[0])?;
        let stop = Self::to_i64(&args[1])?;
        let step = if args.len() == 3 {
            Self::to_i64(&args[2])?
        ...
        let full_count = (stop as i128 - start as i128) / step as i128 + 1;
        let requested = limit.map_or(full_count, |value| value as i128);
        if full_count > MAX_TVF_ROWS as i128 && requested > MAX_TVF_ROWS as i128 {
            return Err(Self::safety_limit_error());
        }
        let count =
            usize::try_from(full_count.min(requested)).map_err(|_| Self::safety_limit_error())?;
        Ok(Box::new((0..count).map(move |index| {
            let value = start as i128 + step as i128 * index as i128;
            Ok((
                index as i64,
                Row::from_values(vec![Value::Integer(value as i64)]),
            ))
        })))
    }

    fn stream_timestamp(args: &[Value], limit: Option<usize>) -> Result<TableRowStream> {
        let start = Self::to_timestamp(&args[0])?;
        let stop = Self::to_timestamp(&args[1])?;
        let step = if args.len() == 3 {
            match &args[2] {
                Value::Text(value) => parse_interval(value)?,
                Value::Integer(days) => chrono::Duration::try_days(*days).ok_or_else(|| {
                    Error::invalid_argument(format!(
                        "GENERATE_SERIES timestamp step {} days overflows",
                        days
                    ))
        ...
        Ok(Box::new(std::iter::from_fn(move || {
            ...
            let value = current?;
            let in_range = if step > chrono::Duration::zero() {
                value <= stop
            } else {
                value >= stop
            };
            ...
            if emitted >= max_rows {
                terminal = true;
                return error_on_exhaustion.then(|| Err(Self::safety_limit_error()));
            }
            current = value.checked_add_signed(step);
            let row_id = emitted as i64;
            emitted += 1;
            Some(Ok((
                row_id,
                Row::from_values(vec![Value::Timestamp(value)]),
            )))
        })))
    }

    fn stream_float(args: &[Value], limit: Option<usize>) -> Result<TableRowStream> {
        let start = Self::to_f64(&args[0])?;
        let stop = Self::to_f64(&args[1])?;
        let step = if args.len() == 3 {
            Self::to_f64(&args[2])?
        ...
        Ok(Box::new(std::iter::from_fn(move || {
            ...
            let row_id = index as i64;
            index += 1;
            Some(Ok((row_id, Row::from_values(vec![Value::Float(value)]))))
        })))
    }
}
...
impl TableValuedFunction for GenerateSeriesFunction {
    ...
    fn info(&self) -> FunctionInfo {
        FunctionInfo::new(
            ...
            "Generate a relation of values from start to stop",
            FunctionSignature::new(FunctionDataType::Any, vec![FunctionDataType::Any], 2, 3),
        )
    }
    ...
    fn stream(&self, args: &[Value], limit: Option<usize>) -> Result<TableRowStream> {
        ...
        if args.iter().take(2).any(Self::is_timestamp_like) {
            Self::stream_timestamp(args, limit)
        } else if args.iter().any(|value| matches!(value, Value::Float(_))) {
            Self::stream_float(args, limit)
        } else {
            Self::stream_integer(args, limit)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn r8_l01_batch_h_generate_series_stream_is_incremental() {
        let series = GenerateSeriesFunction;
        let mut rows = series
            .stream(&[Value::Integer(1), Value::Integer(1_000_000_000)], Some(3))
            .unwrap();
        let values: Vec<i64> = rows
            .by_ref()
            .map(|item| item.unwrap().1.get(0).unwrap().as_int64().unwrap())
            .collect();
