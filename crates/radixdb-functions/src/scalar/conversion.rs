...
fn cast_to_integer(value: &Value) -> Result<Value> {
    match value {
        // Preserve the established SQL CAST unit for timestamps while all
        // numeric admission delegates to the checked core conversion.
        Value::Timestamp(t) => Ok(Value::Integer(t.timestamp())),
        _ => value.try_coerce_to_type(DataType::Integer),
    }
}
...
fn cast_to_float(value: &Value) -> Result<Value> {
    match value {
        ...
        _ => value.try_coerce_to_type(DataType::Float),
    }
}
...
fn cast_to_string(value: &Value) -> Result<Value> {
    match value {
        ...
        Value::Extension(_) => value
            .as_string()
            .map(|text| Value::Text(SmartString::from_string(text)))
            .ok_or_else(|| Error::invalid_argument("Cannot convert extension value to TEXT")),
...
impl ScalarFunction for CollateFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let s = match &args[0] {
            ...
            Value::Extension(_) => args[0].as_string().ok_or_else(|| {
                Error::invalid_argument("COLLATE requires a string-convertible value")
            })?,
...
mod tests {
    ...
    fn test_cast_to_integer() {
        ...
        // Invalid input must not invent zero.
        assert!(cast
            .evaluate(&[Value::text(""), Value::text("INTEGER")])
            .is_err());
    }
    ...
    fn test_cast_text_to_integer_edge_cases() {
        ...
        assert!(cast
            ...
            .is_err());
        assert!(cast
            .evaluate(&[Value::Float(i64::MAX as f64), Value::text("INTEGER"),])
            .is_err());
