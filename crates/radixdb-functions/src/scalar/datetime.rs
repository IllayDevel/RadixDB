...
fn temporal_value(value: &Value, function: &str, argument: &str) -> Result<(DateTime<Utc>, bool)> {
    match value {
        ...
        _ => Err(Error::invalid_argument(format!(
            "{function} {argument} must be a date, timestamp or string"
        ))),
    }
}

fn date_value_from_timestamp(timestamp: DateTime<Utc>, function: &str) -> Result<Value> {
    let days = timestamp.timestamp().div_euclid(86_400);
    let days = i32::try_from(days)
        .map_err(|_| Error::invalid_argument(format!("{function} DATE result is out of range")))?;
    Ok(Value::date(days))
}

fn exact_i64_interval(value: &Value, function: &str) -> Result<i64> {
    match value {
                ...
                && *value < 9_223_372_036_854_775_808.0 =>
        {
            ...
            if integer as f64 == *value {
                ...
                Err(Error::invalid_argument(format!(
                    "{function} interval is not exactly representable as i64"
                )))
            }
        }
        _ => Err(Error::invalid_argument(format!(
            "{function} second argument must be an exact integer"
        ))),
    }
}

fn checked_timestamp_delta(
    timestamp: DateTime<Utc>,
    interval: i64,
    unit: &str,
) -> Result<DateTime<Utc>> {
    let duration = if unit.eq_ignore_ascii_case("day") || unit.eq_ignore_ascii_case("days") {
        Duration::try_days(interval)
    } else if unit.eq_ignore_ascii_case("hour") || unit.eq_ignore_ascii_case("hours") {
        Duration::try_hours(interval)
    } else if unit.eq_ignore_ascii_case("minute") || unit.eq_ignore_ascii_case("minutes") {
        Duration::try_minutes(interval)
    } else if unit.eq_ignore_ascii_case("second") || unit.eq_ignore_ascii_case("seconds") {
        Duration::try_seconds(interval)
    } else if unit.eq_ignore_ascii_case("week") || unit.eq_ignore_ascii_case("weeks") {
        Duration::try_weeks(interval)
    } else {
        return Err(Error::invalid_argument(format!(
            "DATE_ADD invalid unit: {unit}. Valid units are: year(s), month(s), week(s), day(s), hour(s), minute(s), second(s)"
        )));
    }
    .ok_or_else(|| Error::invalid_argument("DATE_ADD interval is out of range"))?;

    timestamp
        .checked_add_signed(duration)
        .ok_or_else(|| Error::invalid_argument("DATE_ADD result timestamp is out of range"))
}
...
impl TimeTruncFunction {
    fn exact_component_nanos(num_str: &str, multiplier: i64) -> Result<i64> {
        let (whole, fraction) = num_str.split_once('.').unwrap_or((num_str, ""));
        if whole.is_empty() && fraction.is_empty() {
            return Err(Error::invalid_argument("Empty duration component"));
        }
        let digits = format!("{}{}", if whole.is_empty() { "0" } else { whole }, fraction);
        let numerator = digits.parse::<i128>().map_err(|_| {
            Error::invalid_argument(format!("Invalid number in duration: {num_str}"))
        })?;
        let scale = 10_i128
            .checked_pow(fraction.len() as u32)
            .ok_or_else(|| Error::invalid_argument("Duration component precision is too large"))?;
        let nanos = numerator
            .checked_mul(i128::from(multiplier))
            .ok_or_else(|| Error::invalid_argument("Duration component is out of range"))?;
        if nanos % scale != 0 {
            return Err(Error::invalid_argument(format!(
                "Duration component is not an exact nanosecond value: {num_str}"
            )));
        }
        i64::try_from(nanos / scale)
            .map_err(|_| Error::invalid_argument("Duration component is out of range"))
    }
    ...
    fn parse_duration(duration_str: &str) -> Result<i64> {
        ...
        while i < chars.len() {
            ...
            let component = Self::exact_component_nanos(&num_str, multiplier)?;
            total_nanos = total_nanos.checked_add(component).ok_or_else(|| {
                Error::invalid_argument("Duration exceeds the supported nanosecond range")
            })?;
        }
...
impl ScalarFunction for TimeTruncFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let duration_nanos = i128::from(duration_nanos);
        let truncated_nanos = ts_nanos.div_euclid(duration_nanos) * duration_nanos;
        let seconds = truncated_nanos.div_euclid(1_000_000_000);
        let nanos = truncated_nanos.rem_euclid(1_000_000_000) as u32;
        let seconds = i64::try_from(seconds)
            .map_err(|_| Error::invalid_argument("TIME_TRUNC result is out of range"))?;
...
/// VERSION function - returns the RadixDB version string
///
...
impl ScalarFunction for VersionFunction {
    ...
    fn info(&self) -> FunctionInfo {
        FunctionInfo::new(
            "VERSION",
            FunctionType::Scalar,
            "Returns the RadixDB database version string",
...
impl ScalarFunction for ExtractFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[1], "EXTRACT", "second argument")?;
...
impl ScalarFunction for YearFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "YEAR", "argument")?;
...
impl ScalarFunction for MonthFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "MONTH", "argument")?;
...
impl ScalarFunction for DayFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "DAY", "argument")?;
...
impl ScalarFunction for HourFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "HOUR", "argument")?;
...
impl ScalarFunction for MinuteFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "MINUTE", "argument")?;
...
impl ScalarFunction for SecondFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts, _) = temporal_value(&args[0], "SECOND", "argument")?;
...
impl ScalarFunction for DateAddFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let interval = exact_i64_interval(&args[1], "DATE_ADD")?;
        ...
        let result = if unit.eq_ignore_ascii_case("year") || unit.eq_ignore_ascii_case("years") {
            ...
            let total_months = (ts.year() as i64)
                .checked_mul(12)
                .and_then(|value| value.checked_add(i64::from(ts.month()) - 1))
                .and_then(|value| value.checked_add(interval))
                .ok_or_else(|| Error::invalid_argument("DATE_ADD month interval overflow"))?;
...
impl ScalarFunction for DateSubFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        if let Value::Integer(i) = &args[1] {
            modified_args[1] = Value::Integer(
                i.checked_neg()
                    .ok_or_else(|| Error::invalid_argument("DATE_SUB interval overflow"))?,
            );
...
impl ScalarFunction for DateDiffFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        let (ts1, _) = temporal_value(&args[0], "DATEDIFF", "first argument")?;
        let (ts2, _) = temporal_value(&args[1], "DATEDIFF", "second argument")?;
...
impl ScalarFunction for CurrentTimestampFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        Ok(Value::Timestamp(system_time_now().into()))
    }
}
...
mod tests {
    ...
    #[test]
    fn time_trunc_uses_euclidean_buckets_and_never_invents_epoch() {
        let function = TimeTruncFunction;
        let before_epoch = Utc.with_ymd_and_hms(1969, 12, 31, 23, 59, 30).unwrap();
        let Value::Timestamp(truncated) = function
            .evaluate(&[Value::text("1m"), Value::Timestamp(before_epoch)])
            .unwrap()
        else {
            panic!("TIME_TRUNC must return a timestamp");
        };
        assert_eq!(
            truncated,
        ...
        let historical = Utc.with_ymd_and_hms(1500, 1, 1, 12, 34, 56).unwrap();
        let Value::Timestamp(truncated) = function
            .evaluate(&[Value::text("1h"), Value::Timestamp(historical)])
            .unwrap()
        else {
            panic!("TIME_TRUNC must return a timestamp");
        };
        assert_eq!(
            truncated,
            Utc.with_ymd_and_hms(1500, 1, 1, 12, 0, 0).unwrap()
        );
    }

    #[test]
    fn date_arithmetic_rejects_unrepresentable_intervals() {
        let timestamp = Value::Timestamp(Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap());
        assert!(DateAddFunction
            .evaluate(&[timestamp.clone(), Value::Integer(i64::MAX)])
            .is_err());
        assert!(DateSubFunction
            .evaluate(&[timestamp, Value::Integer(i64::MIN)])
            .is_err());
    }
