...
/// Convert a serde_json::Value to a radixdb Value
...
impl ScalarFunction for SleepFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        self.evaluate_with_cancellation(args, None)
    }

    fn evaluate_with_cancellation(
        &self,
        args: &[Value],
        cancellation: Option<&dyn crate::FunctionCancellation>,
    ) -> Result<Value> {
        ...
        loop {
            if cancellation.is_some_and(crate::FunctionCancellation::is_cancelled) {
...
mod tests {
    ...
    impl crate::FunctionCancellation for Cancelled {
    ...
    #[test]
    fn sleep_observes_neutral_cancellation_contract() {
        let result =
            SleepFunction.evaluate_with_cancellation(&[Value::Integer(1)], Some(&Cancelled));
        assert_eq!(result, Err(Error::QueryCancelled));
    }
