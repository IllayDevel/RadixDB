...
use radixdb_core::{DataType, Error, Result, SmartString, Value};
...
/// Maximum byte size produced by a single string scalar invocation.
const MAX_STRING_RESULT_SIZE: usize = 10 * 1024 * 1024;

fn push_char_bounded(result: &mut String, ch: char, function: &str) -> Result<()> {
    if result.len().saturating_add(ch.len_utf8()) > MAX_STRING_RESULT_SIZE {
        return Err(Error::invalid_argument(format!(
            "{function} result exceeds the 10 MiB string limit"
        )));
    }
    result.push(ch);
    Ok(())
...
fn take_chars_bounded(value: &str, count: usize, function: &str) -> Result<String> {
    let mut result = String::with_capacity(value.len().min(MAX_STRING_RESULT_SIZE));
    for ch in value.chars().take(count) {
        push_char_bounded(&mut result, ch, function)?;
    }
    Ok(result)
}
...
impl UpperFunction {
    /// Native implementation with in-place mutation
    /// Uses fast ASCII path when possible, falls back to Unicode for non-ASCII
    #[inline]
    ...
    /// For evaluate() - returns a new value
    /// Uses fast ASCII path when possible, falls back to Unicode for non-ASCII
    fn evaluate_impl(arg: &Value) -> Value {
        match arg {
            Value::Text(s) => {
                if s.is_ascii() {
...
impl LengthFunction {
    /// Native implementation with in-place mutation
    /// Returns character count (not byte count) for Unicode correctness
    #[inline]
    fn native(v: &mut Value) {
        let result = match v {
            Value::Text(s) => {
                // Fast path for ASCII - byte length equals char count
            ...
            Value::Null(_) => None,
            _ => {
                let len = v.to_string().chars().count() as i64;
                Some(Value::Integer(len))
            }
        };
...
impl ScalarFunction for RepeatFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        if result_size > MAX_STRING_RESULT_SIZE {
            return Err(Error::invalid_argument(format!(
                "REPEAT would produce {} bytes, exceeding maximum of {} bytes",
                result_size, MAX_STRING_RESULT_SIZE
            )));
        }
...
impl ScalarFunction for LpadFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        if target_len > MAX_STRING_RESULT_SIZE {
            return Err(Error::invalid_argument(
                "LPAD result exceeds the 10 MiB string limit",
            ));
        }
        ...
        if current_len >= target_len {
            let result = take_chars_bounded(&s, target_len, "LPAD")?;
            return Ok(Value::Text(SmartString::from_string(result)));
        }

        let pad_needed = target_len - current_len;
        let mut result = String::with_capacity(target_len.min(MAX_STRING_RESULT_SIZE));

        // Build padding
        let pad_chars: Vec<char> = pad_str.chars().collect();
        if !pad_chars.is_empty() {
            for pad_idx in 0..pad_needed {
                push_char_bounded(&mut result, pad_chars[pad_idx % pad_chars.len()], "LPAD")?;
            }
        }

        if result.len().saturating_add(s.len()) > MAX_STRING_RESULT_SIZE {
            return Err(Error::invalid_argument(
                "LPAD result exceeds the 10 MiB string limit",
            ));
        }
...
impl ScalarFunction for RpadFunction {
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        if target_len > MAX_STRING_RESULT_SIZE {
            return Err(Error::invalid_argument(
                "RPAD result exceeds the 10 MiB string limit",
            ));
        }
        let pad_str = if args.len() == 3 {
            value_to_string(&args[2])
        ...
        let current_len = s.chars().count();
        if current_len >= target_len {
            let result = take_chars_bounded(&s, target_len, "RPAD")?;
            return Ok(Value::Text(SmartString::from_string(result)));
        }

        let pad_needed = target_len - current_len;
        // Pre-allocate with capacity to avoid reallocations
        if s.len() > MAX_STRING_RESULT_SIZE {
            return Err(Error::invalid_argument(
                "RPAD result exceeds the 10 MiB string limit",
            ));
        }
        let mut result = String::with_capacity(
            s.len()
                .saturating_add(pad_needed)
                .min(MAX_STRING_RESULT_SIZE),
        );
        result.push_str(&s);
        ...
        if !pad_chars.is_empty() {
            for pad_idx in 0..pad_needed {
                push_char_bounded(&mut result, pad_chars[pad_idx % pad_chars.len()], "RPAD")?;
            }
        }
...
// ============================================================================
// FROM_HEX - Decode an exact byte string
// ============================================================================

/// Decode a hexadecimal string into the SQL BYTES domain.
///
/// This is deliberately a regular SQL function rather than a dump-only parser
/// escape. Logical dumps can therefore represent every BYTES value while still
/// remaining executable by the ordinary parser and expression evaluator.
#[derive(Default)]
pub struct FromHexFunction;

impl ScalarFunction for FromHexFunction {
    fn name(&self) -> &str {
        "FROM_HEX"
    }
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        validate_arg_count!(args, "FROM_HEX", 1);
        if args[0].is_null() {
            return Ok(Value::null(DataType::Bytes));
        }
        let encoded = value_as_str(&args[0])
            .ok_or_else(|| Error::invalid_argument("FROM_HEX requires a TEXT argument"))?;
        if !encoded.len().is_multiple_of(2) {
            return Err(Error::invalid_argument(
                "FROM_HEX requires an even number of hexadecimal digits",
            ));
        }
        ...
        for (index, pair) in encoded.as_bytes().chunks_exact(2).enumerate() {
            let high = nibble(pair[0]).ok_or_else(|| {
                Error::invalid_argument(format!(
                    "FROM_HEX found a non-hexadecimal digit at byte {}",
                    index * 2
                ))
            })?;
...
mod tests {
    ...
    #[test]
    fn from_hex_preserves_bytes_and_rejects_malformed_input() {
        let function = FromHexFunction;
        assert_eq!(
            function.evaluate(&[Value::text("00fFa5")]).unwrap(),
            Value::bytes(vec![0x00, 0xff, 0xa5])
        );
        assert_eq!(
            function.evaluate(&[Value::text("")]).unwrap(),
            Value::bytes(Vec::new())
        );
        assert_eq!(
            function.evaluate(&[Value::null(DataType::Text)]).unwrap(),
            Value::null(DataType::Bytes)
        );
        assert!(function.evaluate(&[Value::text("abc")]).is_err());
        assert!(function.evaluate(&[Value::text("00xz")]).is_err());
    }

    #[test]
    fn pad_functions_reject_unbounded_results_before_allocation() {
        for function in [
            &LpadFunction as &dyn ScalarFunction,
            &RpadFunction as &dyn ScalarFunction,
        ] {
            assert!(function
                .evaluate(&[Value::text("x"), Value::Integer(i64::MAX)])
                .is_err());
        }
    }
