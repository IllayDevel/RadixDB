        ...
        if let Some(i) = value_to_i64(&args[0]) {
            if matches!(args[0], Value::Integer(_)) {
                return match i.checked_abs() {
                    ...
                    None => Ok(Value::Null(DataType::Integer)), // i64::MIN overflow
                };
            }
        }
    ...
    fn evaluate(&self, args: &[Value]) -> Result<Value> {
        ...
        if let (Some(a), Some(b)) = (value_to_i64(&args[0]), value_to_i64(&args[1])) {
            ...
            return a
                .checked_rem(b)
                .map(Value::Integer)
                .ok_or_else(|| Error::invalid_argument("MOD: integer remainder overflow"));
        }
...
mod tests {
    ...
    #[test]
    fn mod_signed_remainder_overflow_is_an_error() {
        assert!(ModFunction
            .evaluate(&[Value::Integer(i64::MIN), Value::Integer(-1)])
            .is_err());
    }
