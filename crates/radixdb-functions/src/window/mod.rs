...
pub(crate) fn validate_window_position(
    function: &str,
    partition_len: usize,
    current_row: usize,
) -> radixdb_core::Result<()> {
    if partition_len == 0 || current_row >= partition_len {
        return Err(radixdb_core::Error::invalid_argument(format!(
            "{function} current row {current_row} is outside partition length {partition_len}"
        )));
    }
    Ok(())
}
