    ...
    fn process(
        ...
        if n <= 0 {
            return Err(radixdb_core::Error::invalid_argument(
                "NTILE bucket count must be positive",
            ));
        }

        let total_rows = i64::try_from(partition.len())
            .map_err(|_| radixdb_core::Error::invalid_argument("NTILE partition is too large"))?;
        let row_num = i64::try_from(current_row + 1)
            .map_err(|_| radixdb_core::Error::invalid_argument("NTILE row position overflow"))?;
