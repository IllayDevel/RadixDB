...
// - Uses i64::MIN as the in-table empty sentinel and stores that one key out of band
...
// Empty in-table sentinel. The same logical key is stored out of band.
...
impl<V> I64Map<V> {
    ...
    pub fn reserve(&mut self, additional: usize) {
        ...
        self.len = usize::from(self.min_value.is_some());
        self.mask = new_mask;
    ...
    pub fn insert(&mut self, key: i64, value: V) -> Option<V> {
        if key == EMPTY {
            let old = self.min_value.replace(value);
            if old.is_none() {
                self.len += 1;
            }
            return old;
    ...
    pub fn clear(&mut self) {
        if let Some(value) = self.min_value.take() {
            self.len -= 1;
            drop(value);
        }
        for slot in self.slots.iter_mut() {
            if slot.key != EMPTY {
                // Retire the slot and its cardinality before invoking user
                // Drop code. If Drop unwinds, the remaining occupied slots and
                // len still describe the same valid partial map.
                slot.key = EMPTY;
                self.len -= 1;
                // SAFETY: The old non-empty key proved the value initialized.
                unsafe {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (i64, &V)> {
        self.min_value
            ...
            .chain(self.slots.iter().filter_map(|slot| {
                if slot.key != EMPTY {
                    // SAFETY: slot.key != EMPTY means the value is initialized.
    ...
    pub fn iter_mut(&mut self) -> impl Iterator<Item = (i64, &mut V)> {
        self.min_value.iter_mut().map(|value| (EMPTY, value)).chain(
        ...
        F: FnMut(i64, &mut V) -> bool,
    {
        let remove_min = self
            .min_value
            .as_mut()
            .is_some_and(|value| !f(EMPTY, value));
        if remove_min {
            let value = self.min_value.take();
            self.len -= 1;
            drop(value);
    ...
    pub fn entry(&mut self, key: i64) -> Entry<'_, V> {
        if key == EMPTY {
            return if self.min_value.is_some() {
    ...
    pub fn get(&self) -> &V {
        if self.is_min {
            return self
                ...
                .min_value
                .as_ref()
                .expect("occupied minimum entry must have a value");
        }
...
impl<'a, V> VacantEntry<'a, V> {
    ...
    pub fn insert(self, value: V) -> &'a mut V {
        if self.is_min {
            self.map.len += 1;
            return self.map.min_value.insert(value);
        }
...
impl<V> Iterator for IntoIter<V> {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        (
            0,
            Some(self.slots.len() - self.pos + usize::from(self.min_value.is_some())),
        )
    }
}
...
impl I64Set {
    ...
    pub fn iter(&self) -> impl Iterator<Item = i64> + '_ {
        std::iter::once(EMPTY).filter(|_| self.has_min).chain(
            self.slots
                .iter()
    ...
    pub fn drain(&mut self) -> impl Iterator<Item = i64> + '_ {
        let len = self.len;
        let has_min = std::mem::take(&mut self.has_min);
        self.len = 0;
        I64SetDrain {
...
struct I64SetDrain<'a> {
    slots: std::slice::IterMut<'a, i64>,
    has_min: bool,
    remaining: usize,
}

impl Iterator for I64SetDrain<'_> {
    type Item = i64;

    fn next(&mut self) -> Option<Self::Item> {
        if std::mem::take(&mut self.has_min) {
            self.remaining -= 1;
            return Some(EMPTY);
        }
...
impl ExactSizeIterator for I64SetDrain<'_> {}

impl Drop for I64SetDrain<'_> {
    fn drop(&mut self) {
...
mod tests {
    ...
    #[test]
    fn test_mutation_state_before_unwind_i64_map_clear() {
        let panicked = Rc::new(std::cell::Cell::new(false));
        let mut map = std::mem::ManuallyDrop::new(I64Map::new());
        for key in 1..=3 {
            map.insert(
                key,
                PanicOnFirstDrop {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| map.clear()));

        assert!(result.is_err());
        assert_eq!(map.len(), 2, "clear must retire each slot before Drop");
    }
    ...
    fn test_edge_values() {
        ...
        assert_eq!(map.get(i64::MIN), Some(&"min"));
    ...
    #[test]
    fn test_i64_min_full_map_domain() {
        let mut map = I64Map::with_capacity(128);
        assert_eq!(map.insert(i64::MIN, 1), None);
        assert_eq!(map.insert(i64::MIN, 2), Some(1));
        *map.entry(i64::MIN)
            .and_modify(|value| *value += 1)
            .or_insert(0) += 1;
        assert_eq!(map.get(i64::MIN), Some(&4));
        assert_eq!(map.len(), 1);
        ...
        map.shrink_to_fit();
        assert_eq!(map.get(i64::MIN), Some(&4));
        assert!(map.keys().any(|key| key == i64::MIN));
        ...
        assert_eq!(cloned.get(i64::MIN), Some(&5));
        let mut drained: Vec<_> = map.drain().collect();
        drained.sort_by_key(|(key, _)| *key);
        assert_eq!(drained.first(), Some(&(i64::MIN, 5)));
        assert!(map.is_empty());

        let mut owned: Vec<_> = cloned.into_iter().collect();
        owned.sort_by_key(|(key, _)| *key);
        assert_eq!(owned.first(), Some(&(i64::MIN, 5)));

        assert_eq!(map.entry(i64::MIN).or_insert(7), &7);
        assert_eq!(map.remove(i64::MIN), Some(7));
    ...
    #[test]
    fn test_i64set_partial_drain_finishes_on_drop() {
        let mut set = I64Set::new();
        set.insert(1);
        set.insert(2);
        set.insert(3);
    ...
    #[test]
    fn test_i64set_min_full_domain() {
        let mut set = I64Set::with_capacity(128);
        ...
        set.shrink_to_fit();
        assert!(set.contains(i64::MIN));
        assert!(set.iter().any(|key| key == i64::MIN));

        let cloned = set.clone();
        let mut drained: Vec<_> = set.drain().collect();
        drained.sort_unstable();
        assert_eq!(drained.first(), Some(&i64::MIN));
        assert!(set.is_empty());

        let mut owned: Vec<_> = cloned.into_iter().collect();
        owned.sort_unstable();
        assert_eq!(owned.first(), Some(&i64::MIN));
