...
impl<T> CompactVec<T> {
    ...
    #[track_caller]
    fn assert_capacity_representable(capacity: usize) {
        assert!(
            capacity <= Self::MAX_CAPACITY,
            "CompactVec capacity exceeds u32::MAX"
        );
    }
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self::assert_capacity_representable(capacity);
        if capacity == 0 {
            return Self::new();
        }
    ...
    pub fn clear(&mut self) {
        ...
        // SAFETY: All elements [0..len] are initialized. Publish the empty
        // state before invoking user Drop code so unwinding cannot expose an
        // already-dropped element through the vector's bookkeeping.
        unsafe {
            self.set_len(0);
    ...
    pub fn reserve(&mut self, additional: usize) {
        ...
        let required = len
            ...
            .expect("CompactVec capacity exceeds usize::MAX");
        Self::assert_capacity_representable(required);
    ...
    fn grow(&mut self) {
        ...
        let new_cap = if cap == 0 {
            ...
            cap.saturating_mul(2).min(Self::MAX_CAPACITY)
        };
    ...
    fn realloc(&mut self, new_cap: usize) {
        Self::assert_capacity_representable(new_cap);
        let len = self.len();
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        // SAFETY: Elements [len..current_len] are initialized. Publish the new
        // length before invoking user Drop code so unwinding cannot make the
        // truncated tail visible again.
        unsafe {
        ...
        F: FnMut(&T) -> bool,
    {
        ...
        impl<T> Drop for BackshiftOnDrop<'_, T> {
            fn drop(&mut self) {
                // SAFETY: The processed prefix contains `deleted_count` holes.
                // The unprocessed tail is still fully initialized. Shift that
                // tail over the holes and publish the exact surviving length,
                // including when predicate or element Drop unwinds.
                unsafe {
                    if self.deleted_count > 0 {
                        let base = self.vec.ptr.as_ptr();
                        ptr::copy(
                            base.add(self.processed_len),
                            base.add(self.processed_len - self.deleted_count),
        ...
        // SAFETY: The guard owns bookkeeping until it restores the exact
        // surviving length. This prevents CompactVec::drop from observing a
        // partially processed prefix during unwinding.
        unsafe {
            self.set_len(0);
        }
        ...
        while guard.processed_len < original_len {
            // SAFETY: processed_len is inside the original initialized range.
            unsafe {
                let current = guard.vec.ptr.as_ptr().add(guard.processed_len);
                if f(&*current) {
                    if guard.deleted_count > 0 {
                        ptr::copy_nonoverlapping(current, current.sub(guard.deleted_count), 1);
                    }
                    guard.processed_len += 1;
                } else {
                    // Retire the element in bookkeeping before invoking Drop.
                    guard.processed_len += 1;
    ...
    pub fn into_vec(self) -> Vec<T> {
        ...
        if mem::size_of::<T>() == 0 {
            // SAFETY: ZST elements require no allocation. The dangling pointer
            // is aligned and `len` elements are logically initialized.
    ...
    pub fn from_vec(vec: Vec<T>) -> Self {
        Self::assert_capacity_representable(vec.len());
        if mem::size_of::<T>() == 0 {
            let len = vec.len() as u32;
            let mut vec = ManuallyDrop::new(vec);
            return Self {
                ptr: unsafe { NonNull::new_unchecked(vec.as_mut_ptr()) },
                len_cap: Self::pack(len, len),
            };
        }
        Self::assert_capacity_representable(vec.capacity());
        let len = vec.len() as u32;
...
macro_rules! compact_vec {
    () => {
        $crate::CompactVec::new()
    };
    ($($elem:expr),+ $(,)?) => {{
        // Use array to get count at compile time, then collect
        let arr = [$($elem),+];
        let mut vec = $crate::CompactVec::with_capacity(arr.len());
        for elem in arr {
            vec.push(elem);
        }
        vec
...
mod tests {
    ...
    struct PanicOnFirstDrop {
        panicked: std::rc::Rc<std::cell::Cell<bool>>,
    }
    ...
    fn panic_drop_vec(len: usize) -> std::mem::ManuallyDrop<CompactVec<PanicOnFirstDrop>> {
        let panicked = std::rc::Rc::new(std::cell::Cell::new(false));
        std::mem::ManuallyDrop::new(
            (0..len)
                .map(|_| PanicOnFirstDrop {
                    panicked: std::rc::Rc::clone(&panicked),
                })
                .collect(),
        )
    }
    ...
    #[test]
    fn test_mutation_state_before_unwind_compact_vec_clear() {
        let mut vec = panic_drop_vec(3);
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| vec.clear()));

        assert!(result.is_err());
        assert_eq!(vec.len(), 0, "clear must publish empty state before Drop");
    }

    #[test]
    fn test_mutation_state_before_unwind_compact_vec_truncate() {
        let mut vec = panic_drop_vec(3);
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| vec.truncate(1)));

        assert!(result.is_err());
        assert_eq!(vec.len(), 1, "truncate must publish new length before Drop");
    }

    #[test]
    fn test_mutation_state_before_unwind_compact_vec_retain() {
        let mut vec: CompactVec<i32> = [0, 1, 2].into_iter().collect();
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            vec.retain(|value| match *value {
                0 => false,
                1 => panic!("intentional predicate panic"),
                _ => true,
            });
        }));
    ...
    #[test]
    fn test_capacity_above_u32_is_rejected_without_allocation() {
        let unrepresentable = u32::MAX as usize + 1;

        assert!(std::panic::catch_unwind(|| {
            let _ = CompactVec::<()>::with_capacity(unrepresentable);
        })
        .is_err());

        assert!(std::panic::catch_unwind(|| {
            let mut vec = CompactVec::<()>::new();
            vec.reserve(unrepresentable);
        })
        .is_err());

        assert_eq!(Vec::<()>::new().capacity(), usize::MAX);
        let compact = CompactVec::from_vec(Vec::<()>::new());
        assert!(compact.is_empty());
    }

    #[test]
    fn zst_capacity_conversion_and_growth_are_allocation_free() {
        let mut compact = CompactVec::<()>::with_capacity(2);
        assert_eq!(compact.capacity(), 2);
        compact.push(());
        compact.push(());
        compact.push(());
        assert_eq!(compact.len(), 3);
        assert!(compact.capacity() >= 3);

        let standard = compact.into_vec();
        assert_eq!(standard.len(), 3);
        assert_eq!(standard.capacity(), usize::MAX);

        let round_trip = CompactVec::from_vec(standard);
        assert_eq!(round_trip.len(), 3);
        assert_eq!(round_trip.into_vec().len(), 3);
    }
