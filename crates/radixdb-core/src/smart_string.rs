...
pub struct SmartString {
    ...
    tag: StringTag,
    /// Data bytes: inline string data OR (length + `Arc<str>` data pointer)
    /// - 64-bit: 7-byte length + 8-byte pointer at data[7..15]
    /// - 32-bit: 11-byte length area + 4-byte pointer at data[11..15]
    data: [u8; 15],
}
...
impl SmartString {
    ...
    fn new_heap(s: String) -> Self {
        Self::new_heap_arc(Arc::<str>::from(s))
    }

    #[inline]
    fn new_heap_arc(arc: Arc<str>) -> Self {
        let len = arc.len();
        let mut data = [0u8; 15];
        Self::encode_heap_len(&mut data, len);
        let ptr = Arc::into_raw(arc) as *const u8 as usize;
    ...
    #[inline]
    fn encode_heap_len(data: &mut [u8; 15], len: usize) {
        let bytes = len.to_le_bytes();
        let stored = ptr_layout::OFFSET.min(bytes.len());
        assert!(
            bytes[stored..].iter().all(|byte| *byte == 0),
            "SmartString heap length exceeds compact representation"
        );
    ...
    fn heap_len(&self) -> usize {
        ...
        let stored = ptr_layout::OFFSET.min(bytes.len());
        bytes[..stored].copy_from_slice(&self.data[..stored]);
    ...
    pub fn as_str(&self) -> &str {
        if let Some(len) = self.tag.inline_len() {
            ...
            // SAFETY: Heap values store the data pointer and length from a
            // live Arc<str>; get_arc_ptr reconstructs the original fat pointer.
            unsafe { &*self.get_arc_ptr() }
        }
    }
    ...
    /// Reconstruct the raw Arc<str> pointer for heap strings.
    #[inline]
    fn get_arc_ptr(&self) -> *const str {
        ...
        ptr::slice_from_raw_parts(data_ptr, self.heap_len()) as *const str
    }
    ...
    pub fn into_string(self) -> String {
        if self.tag.is_inline() {
            ...
            std::mem::forget(self); // Prevent Drop from decrementing
                                    // SAFETY: ptr is the original Arc<str> raw pointer, and we've
                                    // prevented self's Drop from running, so ownership transfers here.
...
impl Clone for SmartString {
    ...
    fn clone(&self) -> Self {
        if self.tag.is_heap() {
            ...
            // SAFETY: get_arc_ptr() reconstructs the original Arc<str> pointer
            unsafe {
...
impl Drop for SmartString {
    ...
    fn drop(&mut self) {
        if self.tag.is_heap() {
            ...
            let ptr = self.get_arc_ptr();
            // SAFETY: We only store valid Arc<str> pointers
            unsafe {
                Arc::from_raw(ptr);
            }
        }
