...
mod tests {
    ...
    #[test]
    fn test_reused_buffers_guarantee_adjacent_requested_capacity() {
        clear_row_vec_pool();
        clear_row_id_vec_pool();

        {
            let row_vec = RowVec::with_capacity(16);
            assert_eq!(row_vec.capacity(), 16);
        }
        let row_vec = RowVec::with_capacity(17);
        assert!(row_vec.capacity() >= 17);

        {
            let row_ids = RowIdVec::with_capacity(16);
            assert_eq!(row_ids.capacity(), 16);
        }
        let row_ids = RowIdVec::with_capacity(17);
        assert!(row_ids.capacity() >= 17);
    }
