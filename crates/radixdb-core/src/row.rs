...
/// Minimal schema view consumed by the canonical row owner.
///
/// Every index below [`RowSchema::row_column_count`] must resolve to a column.
pub trait RowSchema {
...
impl Row {
    ...
    /// Remove one value by physical column position.
    ///
    /// This is crate-internal because arbitrary callers must not mutate a row's
    /// schema identity. Schema-evolution code uses it while holding the DDL
    /// publication fence to rewrite every hot MVCC version atomically.
    #[inline]
    #[doc(hidden)]
