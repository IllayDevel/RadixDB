...
use radixdb_core::{DataType, Error, Result, Value, ValueSet};
use radixdb_functions::{NativeFn1, ScalarFunction};
...
impl CompiledPattern {
    ...
    pub fn compile(pattern: &str, case_insensitive: bool) -> Result<Self> {
        ...
        if !has_percent && !has_underscore && !has_escape {
            return Ok(CompiledPattern::Exact(pat));
        }
        ...
        if !has_escape {
            ...
            if pat.starts_with('%') && pat.ends_with('%') && pat.len() > 2 {
                ...
                if !middle.contains('%') && !middle.contains('_') {
                    return Ok(CompiledPattern::Contains(middle.to_string()));
                }
            }
        ...
        regex
            ...
            .map_err(|error| Error::invalid_argument(format!("invalid LIKE pattern: {error}")))
    }
    ...
    pub fn compile_glob(pattern: &str) -> Result<Self> {
        ...
        regex
            .map(CompiledPattern::Regex)
            .map_err(|error| Error::invalid_argument(format!("invalid GLOB pattern: {error}")))
    }
...
pub enum Op {
    ...
    /// Call a durable catalog Function through the request-local executor
    /// bridge. Resolution is repeated against the pinned catalog generation.
    CallStored {
        name: CompactArc<str>,
        arg_count: u8,
    },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Op::CallStored { name, arg_count } => {
                write!(f, "CallStored({name}, args={arg_count})")
            }
            ...
            Op::CastExternal(name) => write!(f, "CastExternal({name})"),
...
mod tests {
    ...
    fn test_pattern_match_all() {
        let pattern = CompiledPattern::compile("%", false).unwrap();
    ...
    fn test_pattern_single_char() {
        let pattern = CompiledPattern::compile("_", false).unwrap();
    ...
    fn test_pattern_suffix_case_insensitive() {
        let pattern = CompiledPattern::compile("%World", true).unwrap();
        assert!(pattern.matches("hello world", true));
        assert!(pattern.matches("HELLO WORLD", true));
    ...
    fn test_pattern_contains() {
        let pattern = CompiledPattern::compile("%ello%", false).unwrap();
    ...
    fn test_pattern_contains_case_insensitive() {
        let pattern = CompiledPattern::compile("%ELLO%", true).unwrap();
        assert!(pattern.matches("hello", true));
    ...
    fn test_pattern_prefix_suffix() {
        let pattern = CompiledPattern::compile("hello%world", false).unwrap();
        assert!(
    ...
    fn test_pattern_prefix_suffix_case_insensitive() {
        let pattern = CompiledPattern::compile("Hello%World", true).unwrap();
    ...
    fn test_pattern_prefix_suffix_too_short() {
        let pattern = CompiledPattern::compile("abc%xyz", false).unwrap();
    ...
    fn test_pattern_complex_regex() {
        ...
        let pattern = CompiledPattern::compile("a%b%c", false).unwrap();
    ...
    fn test_pattern_underscore_regex() {
        let pattern = CompiledPattern::compile("a_c", false).unwrap();
        assert!(matches!(pattern, CompiledPattern::Regex(_)));
        assert!(pattern.matches("abc", false));
    ...
    fn test_pattern_mixed_wildcards() {
        let pattern = CompiledPattern::compile("a_%b", false).unwrap();
        assert!(matches!(pattern, CompiledPattern::Regex(_)));
    ...
    #[test]
    fn test_glob_exact() {
        let pattern = CompiledPattern::compile_glob("hello").unwrap();
        assert!(matches!(pattern, CompiledPattern::Exact(ref s) if s == "hello"));
        assert!(pattern.matches("hello", false));
        assert!(!pattern.matches("Hello", false));
    }
    ...
    fn test_glob_match_all() {
        let pattern = CompiledPattern::compile_glob("*").unwrap();
        assert!(matches!(pattern, CompiledPattern::MatchAll));
        assert!(pattern.matches("anything", false));
    }
    ...
    #[test]
    fn test_glob_suffix() {
        let pattern = CompiledPattern::compile_glob("*world").unwrap();
        assert!(matches!(pattern, CompiledPattern::Suffix(ref s) if s == "world"));
        assert!(pattern.matches("world", false));
        assert!(pattern.matches("hello world", false));
    }
    ...
    fn test_glob_contains() {
        let pattern = CompiledPattern::compile_glob("*ello*").unwrap();
        assert!(matches!(pattern, CompiledPattern::Contains(ref s) if s == "ello"));
        assert!(pattern.matches("hello", false));
        assert!(pattern.matches("yellow", false));
    }
    ...
    fn test_glob_prefix_suffix() {
        let pattern = CompiledPattern::compile_glob("hello*world").unwrap();
        assert!(
            matches!(pattern, CompiledPattern::PrefixSuffix(ref p, ref s) if p == "hello" && s == "world")
        );
        assert!(pattern.matches("helloworld", false));
    ...
    fn test_glob_complex_regex() {
        let pattern = CompiledPattern::compile_glob("a*b*c").unwrap();
        assert!(matches!(pattern, CompiledPattern::Regex(_)));
        assert!(pattern.matches("abc", false));
        assert!(pattern.matches("aXbYc", false));
    }
    ...
    fn test_glob_question_mark() {
        let pattern = CompiledPattern::compile_glob("a?c").unwrap();
        assert!(matches!(pattern, CompiledPattern::Regex(_)));
        assert!(pattern.matches("abc", false));
        assert!(!pattern.matches("ac", false));
    }
    ...
    #[test]
    fn invalid_regex_compilation_is_an_error_not_literal_fallback() {
        assert!(CompiledPattern::compile_glob("[?").is_err());
    }
    ...
    fn test_op_debug_like_glob() {
        let pattern = Arc::new(CompiledPattern::compile("test%", false).unwrap());
