...
#[inline]
fn valid_decay_factor(decay_factor: f64) -> f64 {
    if decay_factor.is_finite() && (0.0..=1.0).contains(&decay_factor) {
        decay_factor
    } else {
...
#[inline]
fn apply_finite_correction(base_estimate: u64, correction: f64) -> u64 {
    if !correction.is_finite() || correction <= 0.0 {
        return base_estimate;
    }
    let corrected = base_estimate as f64 * correction;
    if !corrected.is_finite() || corrected >= u64::MAX as f64 {
        u64::MAX
    } else {
...
impl CardinalityFeedback {
    ...
    pub fn update(&mut self, estimated_rows: u64, actual_rows: u64, decay_factor: f64) {
        let decay_factor = valid_decay_factor(decay_factor);
    ...
    pub fn apply_correction(&self, base_estimate: u64) -> u64 {
        ...
        apply_finite_correction(base_estimate, self.correction_factor)
    }
}
...
impl FeedbackCache {
    ...
    pub fn with_settings(decay_factor: f64, max_entries: usize) -> Self {
        Self {
            entries: RwLock::new(FxHashMap::default()),
            decay_factor: valid_decay_factor(decay_factor),
            max_entries,
        }
    }
        ...
        match self.lookup(table_name, predicate_hash) {
            Some(feedback)
                if feedback.is_reliable()
                    && feedback.correction_factor.is_finite()
                    && feedback.correction_factor > 0.0 =>
            {
    ...
    pub fn apply_correction(&self, table_name: &str, predicate_hash: u64, estimate: u64) -> u64 {
        ...
        apply_finite_correction(estimate, correction)
    }
    ...
    pub fn invalidate_table(&self, table_name: &str) {
        self.entries
            .write()
            .unwrap()
            .retain(|(name, _), _| !name.eq_ignore_ascii_case(table_name));
    }
...
fn hash_expression_structure(expr: &Expression, hasher: &mut FxHasher) {
    ...
    match expr {
        ...
        // Literal values are part of the distribution identity. Reusing one
        // correction across unrelated constants is not statistically sound.
        Expression::IntegerLiteral(literal) => {
            "INTEGER_LITERAL".hash(hasher);
            literal.value.hash(hasher);
        }
        Expression::FloatLiteral(literal) => {
            "FLOAT_LITERAL".hash(hasher);
            literal.value.to_bits().hash(hasher);
        }
        ...
        Expression::List(list) => {
            ...
            for element in &list.elements {
                hash_expression_structure(element, hasher);
            }
        }
        Expression::Star(_) => {
            "STAR".hash(hasher);
...
mod tests {
    use super::*;
    use radixdb_sql::ast::{InfixExpression, InfixOperator};
    use radixdb_sql::{Identifier, IntegerLiteral, Position, Token, TokenType};
    ...
    #[test]
    fn test_fingerprint_includes_literal_distribution() {
        // Equal shape with different constants may have unrelated selectivity.
    ...
    #[test]
    fn v2_r5_small_capacity_and_invalid_decay_fail_safe() {
        let disabled = FeedbackCache::with_settings(0.3, 0);
        disabled.record_feedback("t", 1, None, 1, 10);
        assert_eq!(disabled.len(), 0);

        let one = FeedbackCache::with_settings(0.3, 1);
        one.record_feedback("t", 1, None, 1, 10);
        one.record_feedback("t", 2, None, 1, 10);
        assert_eq!(one.len(), 1);

        for invalid in [f64::NAN, f64::INFINITY, -1.0, 2.0] {
            let cache = FeedbackCache::with_settings(invalid, 4);
            cache.record_feedback("t", 1, None, 10, 100);
            cache.record_feedback("t", 1, None, 10, 100);
            let corrected = cache.apply_correction("t", 1, 10);
            assert!((1..=1000).contains(&corrected));
        }

        let mut poisoned = CardinalityFeedback::new(1, "t", None, 10, 10);
        poisoned.sample_count = MIN_SAMPLE_COUNT;
        poisoned.correction_factor = f64::NAN;
        assert_eq!(poisoned.apply_correction(77), 77);
    }
}
