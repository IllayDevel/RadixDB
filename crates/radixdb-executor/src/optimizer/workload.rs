...
//!
//! These features make RadixDB unique in that it learns from your specific workload
...
impl WorkloadLearner {
    ...
    fn is_learning_enabled(&self) -> bool {
        self.config
            .read()
            .map(|config| config.learning_enabled)
            .unwrap_or(false)
    }
...
impl EdgeAwarePlanner {
    ...
    pub fn from_global() -> Self {
        ...
        Self {
            hints,
            memory_limit: config.memory_limit_mb.saturating_mul(1024 * 1024),
        }
    }
    ...
    pub fn should_stream(&self, estimated_rows: u64) -> bool {
        ...
        if self.hints.memory_constrained {
            ...
            estimated_rows.saturating_mul(row_size_estimate) > self.memory_limit / 2
        } else {
            false
        }
    }
    ...
    pub fn recommend_join_for_edge(
        ...
        let build_memory = build_rows.saturating_mul(memory_per_build_row);
...
mod tests {
    ...
    #[test]
    fn v2_r5_config_is_authoritative_and_memory_math_is_conservative() {
        let learner = WorkloadLearner::new();
        let mut config = learner.config();
        config.learning_enabled = false;
        config.edge_mode = EdgeMode::Constrained;
        config.memory_limit_mb = u64::MAX;
        learner.set_config(config);
        learner.record_query(
            1,
            QueryPattern::PointLookup,
        ...
        let planner = EdgeAwarePlanner::new(
            WorkloadHints {
                memory_constrained: true,
        ...
        assert!(planner.should_stream(u64::MAX));
        assert!(matches!(
            planner.recommend_join_for_edge(u64::MAX, 1, u64::MAX),
            EdgeJoinRecommendation::ForceNestedLoop { .. }
        ));
    }
