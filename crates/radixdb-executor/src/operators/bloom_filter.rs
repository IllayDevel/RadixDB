...
use crate::hash_table::hash_keys_with;
use crate::operator::{ColumnInfo, Operator, RowRef};
use crate::optimizer::bloom::{BloomEffectivenessTracker, RuntimeBloomFilter};
    ...
    fn next(&mut self) -> Result<Option<RowRef>> {
        ...
        while let Some(row_ref) = self.child.next()? {
            ...
            let has_null = self
                .key_indices
                .iter()
                .any(|&idx| row_ref.get(idx).is_none_or(Value::is_null));
            ...
            if self.bloom_filter.might_match_raw_hash(hash) {
                ...
                BloomEffectivenessTracker::global().record_passed();
                return Ok(Some(row_ref));
            }
            // Definitely no match - skip this row.
            BloomEffectivenessTracker::global().record_rejected();
        }
...
mod tests {
    use super::*;
    use crate::hash_table::hash_row_keys;
    ...
    #[test]
    fn test_bloom_filter_operator_passes_typed_null_key() {
        let builder = BloomFilterBuilder::new("id".to_string(), "test".to_string(), 1);
        let bloom = builder.build();
        let probe_rows = vec![Row::from_values(vec![Value::Null(
        ...
        op.open().unwrap();
        let row = op.next().unwrap().expect("typed NULL must pass through");
