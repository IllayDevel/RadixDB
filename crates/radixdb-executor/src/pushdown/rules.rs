...
use radixdb_functions::{global_registry, ScalarFunction};
...
use radixdb_storage::expression::{
    ...
    FunctionExpr, InListExpr, LikeExpr, NotExpr, NullCheckExpr, OrExpr, ScalarEvaluator,
};
use std::sync::Arc;
...
struct ScalarFunctionEvaluator {
    function: Box<dyn ScalarFunction>,
}

impl ScalarEvaluator for ScalarFunctionEvaluator {
    fn name(&self) -> &str {
        self.function.name()
    }

    fn evaluate(&self, args: &[Value]) -> radixdb_core::Result<Value> {
        self.function.evaluate(args)
    }
}

fn bind_scalar_evaluator(function: Box<dyn ScalarFunction>) -> Arc<dyn ScalarEvaluator> {
    Arc::new(ScalarFunctionEvaluator { function })
}
...
fn extract_literal_with_ctx(expr: &ast::Expression, ctx: &PushdownContext<'_>) -> Option<Value> {
    ...
    match expr {
        ast::Expression::Infix(_) | ast::Expression::Prefix(_) => {
            crate::expression::try_eval_constant_expr(expr)
        }
...
/// Storage comparison expressions have a deliberately smaller scalar domain
/// than `Value`. Publishing an unsupported typed predicate as `Converted`
/// would suppress the authoritative executor residual.
fn supports_comparison_pushdown(data_type: DataType) -> bool {
    matches!(
        data_type,
        DataType::Integer
            | DataType::Float
...
impl PushdownRule for ComparisonRule {
    ...
    fn try_convert(&self, expr: &ast::Expression, ctx: &PushdownContext<'_>) -> PushdownResult {
        ...
        if !ctx
            .column_type(&column)
            .is_some_and(supports_comparison_pushdown)
        {
...
impl PushdownRule for LogicalAndRule {
    ...
    fn try_convert(&self, expr: &ast::Expression, ctx: &PushdownContext<'_>) -> PushdownResult {
        ...
        let left_plan = registry().try_pushdown_plan_with_ctx(&infix.left, ctx);
        let right_plan = registry().try_pushdown_plan_with_ctx(&infix.right, ctx);
        let residual = match (left_plan.residual, right_plan.residual) {
            (Some(left), Some(right)) => {
                let mut residual = infix.clone();
                residual.left = Box::new(left);
                residual.right = Box::new(right);
                Some(ast::Expression::Infix(residual))
            }
            (Some(residual), None) | (None, Some(residual)) => Some(residual),
            (None, None) => None,
        };

        match (left_plan.storage_expr, right_plan.storage_expr) {
            ...
            (Some(expr), None) | (None, Some(expr)) => PushdownResult::Partial {
                storage_expr: expr,
                residual: residual.expect("missing residual for partial AND pushdown"),
            },
            (None, None) => {
...
impl PushdownRule for BetweenRule {
    ...
    fn try_convert(&self, expr: &ast::Expression, ctx: &PushdownContext<'_>) -> PushdownResult {
        ...
        if !ctx
            .column_type(&column)
            .is_some_and(supports_between_pushdown)
        {
            return PushdownResult::CannotPush;
        }

        let lower = match extract_literal_with_ctx(&between.lower, ctx) {
            Some(v) => match ctx.coerce_to_column_type(&column, v) {
                Some(value) => value,
                None => return PushdownResult::CannotPush,
            },
            None => return PushdownResult::CannotPush,
...
impl PushdownRule for InListRule {
    ...
    fn try_convert(&self, expr: &ast::Expression, ctx: &PushdownContext<'_>) -> PushdownResult {
        ...
        let coerced_values: Option<Vec<Value>> = values
            .into_iter()
            .map(|v| ctx.coerce_to_column_type(&column, v))
            .collect();
        let Some(coerced_values) = coerced_values else {
            return PushdownResult::CannotPush;
        };
...
impl LikeRule {
    fn convert_like_expr(
        ...
        let column = if let Some(col) = extract_column_name(&like.left) {
            col
        } else {
            return PushdownResult::CannotPush;
        };
    ...
    fn convert_infix_like(
        ...
        let column = if let Some(col) = extract_column_name(&infix.left) {
            col
        } else {
            return PushdownResult::CannotPush;
        };
        ...
        let (is_ilike, is_not) = match infix.op_type {
            InfixOperator::Like => (false, false),
            InfixOperator::ILike => (true, false),
            InfixOperator::NotLike => (false, true),
...
impl FunctionRule {
    ...
    fn try_extract_function_comparison(
        ...
        // Volatile / side-effecting functions must remain in the executor's
        // checked memory-filter path. Pushing them into storage would either
        // freeze per-row semantics or let storage fast predicates turn runtime
        // errors into a silent `false`.
        if contains_non_deterministic_volatile(func_side) {
            return None;
        }
        ...
        for arg in &func_call.arguments {
            let fa = self.convert_func_arg(arg, ctx)?;
            args.push(fa);
        }
        ...
        // Create FunctionExpr
        let mut func_expr = FunctionExpr::new(
            bind_scalar_evaluator(scalar_func),
            args,
            operator,
            compare_value,
        );
        ...
        if let ast::Expression::FunctionCall(fc) = expr {
            ...
            if let Some(scalar_func) = global_registry().get_scalar(&func_name) {
                ...
                return Some(FunctionArg::Function {
                    function: bind_scalar_evaluator(scalar_func),
