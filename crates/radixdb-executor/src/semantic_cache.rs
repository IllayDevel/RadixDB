...
use radixdb_core::{Result, Row, Value, ValueSet};
...
use radixdb_sql::ast::{Expression, InfixOperator};
...
/// Maximum estimated retained bytes across all semantic-cache entries.
pub const DEFAULT_MAX_GLOBAL_CACHED_BYTES: usize = 256 * 1024 * 1024;
...
pub struct CachedResult {
    ...
    /// Conservative estimate of memory retained by the cached rows.
...
impl CachedResult {
    ...
    pub fn new(
        ...
        let now = Instant::now();
        let estimated_bytes = estimate_cached_rows_bytes(&rows);
        Self {
            fingerprint,
            column_names,
...
pub struct SemanticCache {
    ...
    /// Maximum estimated retained bytes across all entries.
    max_global_bytes: usize,
    /// Current total row count across all entries (for global limit enforcement)
    global_row_count: AtomicU64,
    /// Current estimated retained bytes across all entries.
    global_byte_count: AtomicU64,
    /// Changes whenever data/schema invalidation makes in-flight results stale.
...
fn estimate_cached_rows_bytes(rows: &[Row]) -> usize {
    rows.iter().fold(std::mem::size_of_val(rows), |total, row| {
        row.iter().fold(
            total.saturating_add(std::mem::size_of::<Row>()),
            |row_total, value| {
...
impl SemanticCache {
    ...
    pub fn with_config(
        ...
        Self::with_config_and_byte_limit(
            max_size,
            ttl,
            max_rows,
            max_global_rows,
            DEFAULT_MAX_GLOBAL_CACHED_BYTES,
        )
    }

    /// Create a semantic cache with explicit row and byte budgets.
    pub fn with_config_and_byte_limit(
        max_size: usize,
        ttl: Duration,
        max_rows: usize,
        max_global_rows: usize,
        ...
        Self {
            ...
            global_byte_count: AtomicU64::new(0),
    ...
    pub fn insert_if_generation(
        ...
        if self.generation() != expected_generation || rows.len() > self.max_rows {
            return;
        }
        let new_row_count = rows.len();
        let (table_key, column_key) = Self::cache_keys(table_name, &columns);
        let fingerprint = match &predicate {
            Some(predicate) => {
                QueryFingerprint::with_predicate(table_name, columns.clone(), predicate)
            }
            None => QueryFingerprint::new(table_name, columns.clone()),
        };
        let entry = CachedResult::new(fingerprint, columns, rows, predicate);
    ...
    /// Shared generation-fenced insert logic.
    fn insert_entry(
        &self,
        entry: CachedResult,
        new_row_count: usize,
        ...
        if self.max_size == 0
            || self.max_rows == 0
            || self.max_global_rows == 0
            || self.max_global_bytes == 0
        {
            return;
        }
        let new_byte_count = entry.estimated_bytes;
        if new_row_count > self.max_global_rows || new_byte_count > self.max_global_bytes {
            return;
        }
        ...
        // Enforce both budgets across every table, including the table being
        // inserted. Protecting that table allowed one wide table to grow
        // without bound.
        let current_global = self.global_row_count.load(Ordering::Relaxed) as usize;
        let current_bytes = self.global_byte_count.load(Ordering::Relaxed) as usize;
        let rows_to_free = (current_global + new_row_count).saturating_sub(self.max_global_rows);
        let bytes_to_free = (current_bytes + new_byte_count).saturating_sub(self.max_global_bytes);
        if rows_to_free > 0 || bytes_to_free > 0 {
            self.evict_global_lru(&mut cache, rows_to_free, bytes_to_free);
        }
        ...
        while entries.len() >= self.max_size {
                ...
                .min_by_key(|(_, e)| (e.last_accessed, e.access_count))
            {
                ...
                bytes_freed += entries[idx].estimated_bytes;
                entries.remove(idx);
        ...
        self.global_byte_count
            .fetch_add(new_byte_count as u64, Ordering::Relaxed);
        entries.push(entry);
    }

    /// Evict entries across all tables until both global budgets are satisfied.
    fn evict_global_lru(
        &self,
        cache: &mut StringMap<StringMap<Vec<CachedResult>>>,
        mut rows_to_free: usize,
        mut bytes_to_free: usize,
    ) {
        while rows_to_free > 0 || bytes_to_free > 0 {
            // Find the globally oldest entry across all tables
            let mut oldest: Option<(String, String, usize, Instant, u64, usize, usize)> = None;
                ...
                for (col_key, entries) in table_cache.iter() {
                    for (idx, entry) in entries.iter().enumerate() {
                        let dominated = match &oldest {
                            None => true,
                            Some((_, _, _, last_acc, acc_count, _, _)) => {
            ...
            match oldest {
                Some((table_key, col_key, idx, _, _, row_count, byte_count)) => {
                    if let Some(table_cache) = cache.get_mut(&table_key) {
                        if let Some(entries) = table_cache.get_mut(&col_key) {
                            ...
                            bytes_to_free = bytes_to_free.saturating_sub(byte_count);
            ...
            Ok(mut cache) => {
                ...
                if let Some(table_cache) = cache.get(table_key.as_ref()) {
                    let rows_removed: usize = table_cache
                        ...
                        .map(|e| e.rows.len())
                        .sum();
                    let bytes_removed: usize = table_cache
                        .values()
                        .flat_map(|entries| entries.iter())
                    ...
                    if bytes_removed > 0 {
                        self.global_byte_count
                            .fetch_sub(bytes_removed as u64, Ordering::Relaxed);
                    }
                }
    ...
    pub fn clear(&self) {
        ...
        self.global_byte_count.store(0, Ordering::Relaxed);
...
fn check_range_subsumption(cached: &Expression, new: &Expression) -> Option<SubsumptionResult> {
    ...
    let cached_val = extract_literal_value(&cached_infix.right)?;
    let new_val = extract_literal_value(&new_infix.right)?;
    let bound_order = new_val.cmp(&cached_val);
    ...
    match (&cached_infix.op_type, &new_infix.op_type) {
        ...
        (
            ...
            std::cmp::Ordering::Greater => Some(SubsumptionResult::Subsumed {
                filter: Box::new(new.clone()),
            }),
            std::cmp::Ordering::Less => None,
            std::cmp::Ordering::Equal => match (&cached_infix.op_type, &new_infix.op_type) {
                (InfixOperator::GreaterEqual, InfixOperator::GreaterThan) => {
                    Some(SubsumptionResult::Subsumed {
                        filter: Box::new(new.clone()),
                    })
                }
                (cached_op, new_op) if cached_op == new_op => Some(SubsumptionResult::Identical),
                _ => None,
            },
        },
        ...
        (
            ...
            std::cmp::Ordering::Less => Some(SubsumptionResult::Subsumed {
                filter: Box::new(new.clone()),
            }),
...
fn check_in_subsumption(cached: &Expression, new: &Expression) -> Option<SubsumptionResult> {
    ...
    // NOT IN has the opposite set-containment direction and NULL-sensitive
    // three-valued semantics. Keep it fail-closed unless AST equality already
    // returned Identical above.
    ...
    let is_subset = new_values.iter().all(|value| cached_values.contains(value));
...
fn extract_in_values(expr: &Expression) -> Option<ValueSet> {
    match expr {
        Expression::List(list) => list.elements.iter().map(extract_literal_value).collect(),
...
mod tests {
    use super::*;
    use radixdb_sql::ast::{
        FloatLiteral, Identifier, InExpression, InfixExpression, ListExpression,
    };
    use radixdb_sql::token::{Position, Token, TokenType};

    fn parsed_where(sql: &str) -> Expression {
    ...
    #[test]
    fn r5_l03_semantic_and_classification_identity_preserve_complete_ast_semantic() {
        let cases = [
            ("x > 100", "x >= 100"),
            ("x < 100", "x <= 100"),
            ("x IN (1)", "x NOT IN (1)"),
        ...
        for (cached, new) in cases {
            let cached = parsed_where(&format!("SELECT * FROM t WHERE {cached}"));
            let new = parsed_where(&format!("SELECT * FROM t WHERE {new}"));
            assert!(matches!(
                check_subsumption(Some(&cached), Some(&new)),
                SubsumptionResult::NoSubsumption
            ));
        }

        let cached = parsed_where("SELECT * FROM t WHERE x IN (1, 1, 2)");
        let same_set = parsed_where("SELECT * FROM t WHERE x IN (2, 1)");
        assert!(matches!(
            check_subsumption(Some(&cached), Some(&same_set)),
            SubsumptionResult::Identical
        ));
    }

    #[test]
    fn r5_l03_cache_budgets_and_lru_follow_runtime_usage_semantic() {
        let byte_bounded =
            SemanticCache::with_config_and_byte_limit(8, Duration::from_secs(60), 8, 8, 128);
        byte_bounded.insert(
            "wide",
            vec!["payload".to_string()],
            vec![Row::from_values(vec![Value::Text("x".repeat(1024).into())])],
            None,
        );
        assert!(matches!(
            byte_bounded.lookup("wide", &["payload".to_string()], None),
            CacheLookupResult::Miss
        ));

        let row_bounded = SemanticCache::with_config_and_byte_limit(
            8,
        ...
        assert!(matches!(
            row_bounded.lookup("same_table", &["a".to_string()], None),
            CacheLookupResult::Miss
        ));
        assert!(matches!(
            row_bounded.lookup("same_table", &["b".to_string()], None),
            CacheLookupResult::ExactHit(_)
        ));
    }
    ...
    fn v2_r5_zero_and_one_capacity_are_hard_bounds() {
        let disabled = SemanticCache::with_config(0, Duration::from_secs(60), 10, 10);
        disabled.insert(
            "t",
            vec!["id".to_string()],
            vec![Row::from_values(vec![Value::Integer(1)])],
        ...
        let one = SemanticCache::with_config(1, Duration::from_secs(60), 10, 10);
        one.insert(
            "t",
            vec!["id".to_string()],
            vec![Row::from_values(vec![Value::Integer(1)])],
        ...
        one.insert(
            "t",
            vec!["id".to_string()],
            vec![Row::from_values(vec![Value::Integer(2)])],
            Some(make_gt("id", 1)),
        );
    ...
    #[test]
    fn v2_r5_adjacent_floats_are_not_exact_cache_hits() {
        let left = Expression::FloatLiteral(FloatLiteral {
            token: Token::new(TokenType::Float, "0.0", Position::default()),
            value: 0.0,
        });
        let right = Expression::FloatLiteral(FloatLiteral {
            token: Token::new(TokenType::Float, "5e-324", Position::default()),
