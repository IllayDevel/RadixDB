...
if systemctl is-active --quiet radixdb.service; then
  ...
  [[ "${production_pid}" =~ ^[1-9][0-9]*$ ]] || {
    echo "production RadixDB has no active process" >&2; exit 69;
  }
  production_exe="$(readlink -f "/proc/${production_pid}/exe")"
  [[ "${production_exe}" == /opt/radixdb/* ]] || {
    echo "production executable moved outside /opt/radixdb" >&2; exit 69;
  }
  ss -ltnp 'sport = :15441' | grep -F "pid=${production_pid}," >/dev/null || {
    echo "production listener identity changed" >&2; exit 69;
  }
else
  [[ "${allow_inactive_production}" == 1 ]] || {
    echo "production RadixDB is not active" >&2; exit 69;
  }
...
echo "started isolated soak run ${run_id} profile=${profile}"
