...
fn validate_cast_type_name(type_name: &str) -> Result<(), String> {
    let upper = type_name.to_ascii_uppercase();
    let (base, arguments) = match upper.split_once('(') {
        Some((base, suffix)) => {
            let Some(arguments) = suffix.strip_suffix(')') else {
                return Err(format!("invalid CAST target type: {type_name}"));
            };
            (base, Some(arguments))
        }
    ...
    // External types are deliberately schema-qualified. Their existence and
    // identity are catalog concerns resolved by the executor, not parser
    // keywords maintained beside the catalog.
    if arguments.is_none()
        && base.contains('.')
        && base.split('.').all(|component| !component.is_empty())
    {
        return Ok(());
    }
    ...
    if !matches!(
        ...
        return Err(format!("unknown CAST target type: {type_name}"));
    }

    let Some(arguments) = arguments else {
        return Ok(());
    };
    let values: Vec<&str> = arguments.split(',').map(str::trim).collect();
    let parse_positive = |value: &str| value.parse::<usize>().ok().filter(|&parsed| parsed > 0);

    match base {
        "VARCHAR" | "CHAR" => {
            if values.len() == 1 && parse_positive(values[0]).is_some() {
                Ok(())
            } else {
                Err(format!("{base} requires one positive length"))
            }
        }
        "DECIMAL" | "NUMERIC" => {
            let Some(precision) = values.first().and_then(|value| parse_positive(value)) else {
                return Err(format!("{base} requires a positive precision"));
            };
            if values.len() > 2 || precision > 38 {
                return Err(format!("invalid {base} precision/scale: {arguments}"));
            }
            if let Some(scale) = values.get(1).and_then(|value| value.parse::<usize>().ok()) {
                if scale <= precision {
                    Ok(())
                } else {
                    Err(format!("{base} scale cannot exceed precision"))
                }
            } else if values.len() == 1 {
                Ok(())
            } else {
                Err(format!("invalid {base} precision/scale: {arguments}"))
            }
        }
        "VECTOR" => {
            if values.len() == 1
                && parse_positive(values[0]).is_some_and(|dimension| dimension <= u16::MAX as usize)
            {
                Ok(())
            } else {
                Err("VECTOR requires one dimension between 1 and 65535".to_string())
            }
        }
        _ => Err(format!("type modifiers are not supported for {base}")),
    }
}

fn window_frame_rank(bound: &WindowFrameBound) -> i128 {
    match bound {
        WindowFrameBound::UnboundedPreceding => i128::MIN,
        WindowFrameBound::Preceding(expression) => match expression.as_ref() {
            Expression::IntegerLiteral(value) => -(value.value as i128),
            _ => unreachable!("window offsets are validated before frame ordering"),
        },
        WindowFrameBound::CurrentRow => 0,
        WindowFrameBound::Following(expression) => match expression.as_ref() {
            Expression::IntegerLiteral(value) => value.value as i128,
            _ => unreachable!("window offsets are validated before frame ordering"),
        },
...
fn window_frame_is_ordered(start: &WindowFrameBound, end: Option<&WindowFrameBound>) -> bool {
    if matches!(start, WindowFrameBound::UnboundedFollowing)
        || end.is_some_and(|bound| matches!(bound, WindowFrameBound::UnboundedPreceding))
    {
        return false;
    }
    window_frame_rank(start)
        <= end
            .map(window_frame_rank)
            .unwrap_or_else(|| window_frame_rank(&WindowFrameBound::CurrentRow))
}

impl Parser {
    /// Parse an expression with the given precedence
    pub fn parse_expression(&mut self, precedence: Precedence) -> Option<Expression> {
        if !self.enter_expression() {
            return None;
        }
    ...
    fn parse_float_literal(&mut self) -> Option<Expression> {
        self.parse_finite_float_value().map(|value| {
    ...
    /// Parse a finite f64 without silently inventing infinity or zero.
    fn parse_finite_float_value(&mut self) -> Option<f64> {
        let source = self.cur_token.literal.as_str();
        match source.parse::<f64>() {
            Ok(value) if !value.is_finite() => {
                self.add_error(format!(
                    "float literal {} is out of finite f64 range",
                    source
                ));
                None
            }
            Ok(value)
                if value == 0.0
                    && source.split(['e', 'E']).next().is_some_and(|mantissa| {
                        mantissa.bytes().any(|b| matches!(b, b'1'..=b'9'))
                    }) =>
            {
                self.add_error(format!("float literal {} underflows finite f64", source));
                None
            }
            Ok(value) => Some(value),
            Err(e) => {
                self.add_error(format!("could not parse {} as float: {}", source, e));
                None
            }
        }
    }
    ...
    fn parse_parameter(&mut self) -> Option<Expression> {
        ...
        let index = if name == "?" {
            if self.positional_parameter_style
                == Some(super::parser::PositionalParameterStyle::Explicit)
            {
                self.add_error("cannot mix '?' and '$n' positional parameters".to_string());
                return None;
            }
            self.positional_parameter_style =
                Some(super::parser::PositionalParameterStyle::Anonymous);
            ...
            match stripped.parse::<usize>() {
                Ok(0) => {
                    self.add_error(
                        "positional parameter indexes start at $1; $0 is invalid".to_string(),
                    );
                    return None;
                }
                Ok(idx) => idx,
    ...
    fn parse_unary_expression(&mut self) -> Option<Expression> {
            ...
            && (self.cur_token_is(TokenType::Integer) || self.cur_token_is(TokenType::Float))
        {
            if self.cur_token_is(TokenType::Integer) {
                ...
                let value = self.parse_finite_float_value()?;
    ...
    fn parse_binary_expression(&mut self, left: Expression) -> Option<Expression> {
            ...
            || self.cur_token_is_keyword("SOME")
        {
            if !matches!(
                ...
                self.add_error(format!(
                    "{} is not a valid comparison operator before {}",
    ...
    /// Parse a keyword infix expression (AND, OR, LIKE, IS, IN, BETWEEN)
        ...
        match keyword.as_str() {
            ...
            "NOT" => {
                ...
                if self.peek_token_is_keyword("IN") {
                    ...
                    self.parse_like_expression(left, SmartString::const_new("LIKE"), true)
                } else if self.peek_token_is_keyword("ILIKE") {
                    self.next_token(); // consume ILIKE
                    self.parse_like_expression(left, SmartString::const_new("ILIKE"), true)
                } else if self.peek_token_is_keyword("GLOB") {
                    self.next_token(); // consume GLOB
                    self.parse_like_expression(left, SmartString::const_new("GLOB"), true)
                } else if self.peek_token_is_keyword("REGEXP") {
                    self.next_token(); // consume REGEXP
                    self.parse_like_expression(left, SmartString::const_new("REGEXP"), true)
                } else if self.peek_token_is_keyword("RLIKE") {
                    self.next_token(); // consume RLIKE
                    self.parse_like_expression(left, SmartString::const_new("RLIKE"), true)
                } else {
                    self.add_error(format!(
                        "NOT must be followed by IN, BETWEEN, LIKE, ILIKE, GLOB, REGEXP, or RLIKE at {}",
    ...
    /// Parse a qualified identifier, unresolved multi-part path, or qualified
    /// star. Schema meaning is intentionally not assigned by the parser.
    fn parse_qualified_identifier(&mut self, left: Expression) -> Option<Expression> {
        if let Expression::Parameter(mut parameter) = left {
            if parameter.index != 0 || !parameter.name.starts_with(':') || parameter.field.is_some()
            {
                self.add_error(format!(
                    "only one field may follow a named parameter at {}",
            ...
            self.next_token();
            parameter.field = Some(Box::new(Identifier::new(
                self.cur_token.clone(),
                self.cur_token.literal.clone(),
            )));
            return Some(Expression::Parameter(parameter));
        }
        let mut qualified = match left {
            Expression::Identifier(id) => QualifiedIdentifier {
                token: id.token.clone(),
                qualifier: Box::new(id),
                intermediate: None,
            ...
            Expression::QualifiedIdentifier(path) => path,
            _ => {
                self.add_error(format!(
                    "left side of '.' must be an identifier path at {}",
                    self.cur_token.position
                ));
                return None;
            }
        ...
        if self.peek_token_is_operator("*") {
            if qualified.is_multi_part_path() || !qualified.name.value.is_empty() {
                self.add_error(format!(
                    "qualified star cannot follow a multi-part identifier path at {}",
                    self.cur_token.position
                ));
                return None;
            }
            ...
            return Some(Expression::QualifiedStar(QualifiedStarExpression {
                token: qualified.qualifier.token.clone(),
                qualifier: qualified.qualifier.value,
            }));
        }
        ...
        self.next_token();
        let next = Identifier::new(self.cur_token.clone(), self.cur_token.literal.clone());
        if !qualified.name.value.is_empty() {
            qualified
                .intermediate
                .get_or_insert_with(|| Box::new(Vec::new()))
                .push(*qualified.name);
        }
        qualified.name = Box::new(next);

        Some(Expression::QualifiedIdentifier(qualified))
    }
    ...
    fn parse_window_frame(&mut self) -> Option<WindowFrame> {
        ...
        if !window_frame_is_ordered(&start, end.as_ref()) {
            self.add_error("window frame start must not follow its end".to_string());
            return None;
        }
    ...
    fn parse_window_frame_bound(&mut self) -> Option<WindowFrameBound> {
        if self.cur_token_is_keyword("CURRENT") {
            ...
            if !matches!(&expr, Expression::IntegerLiteral(value) if value.value >= 0) {
                self.add_error(
                    "window frame offset must be a non-negative integer literal".to_string(),
                );
                return None;
            }
            self.next_token();
    ...
    fn parse_like_expression(
        ...
        let token = self.cur_token.clone();
        let operator = if not {
            SmartString::from_string(format!("NOT {}", op))
        } else {
            op
        };
        let precedence = self.cur_precedence();
        ...
        let escape = if self.peek_token_is_keyword("ESCAPE") {
            ...
            let escape = self.parse_expression(Precedence::Lowest)?;
            match &escape {
                Expression::StringLiteral(lit) if lit.value.chars().count() == 1 => {}
                _ => {
                    self.add_error(
                        "LIKE ESCAPE must be a string literal containing exactly one character"
                            .to_string(),
                    );
                    return None;
                }
    ...
    fn parse_cast_expression(&mut self) -> Option<Expression> {
        ...
        let type_name = self.parse_column_data_type()?;
        if let Err(message) = validate_cast_type_name(&type_name) {
    ...
    fn parse_interval_literal(&mut self) -> Option<Expression> {
        ...
        if self.peek_token_is(TokenType::Integer) {
            ...
            let unit_raw = parts[1].to_ascii_uppercase();
            let unit = match unit_raw.as_str() {
                "SECOND" | "SECONDS" => "second",
                "MINUTE" | "MINUTES" => "minute",
                "HOUR" | "HOURS" => "hour",
                ...
                _ => {
                    self.add_error(format!(
                        "invalid interval unit: {} at {}. Expected SECOND, MINUTE, HOUR, DAY, WEEK, MONTH, or YEAR",
                        parts[1], self.cur_token.position
                    ));
                    return None;
                }
            };
    ...
    pub fn parse_group_by_clause(&mut self) -> GroupByClause {
        use crate::ast::{GroupByClause, GroupByModifier};
    ...
    pub fn parse_order_by_expression(&mut self) -> Option<OrderByExpression> {
        ...
        let nulls_first = if self.peek_token_is_keyword("NULLS") {
            ...
            if self.peek_token_is_keyword("FIRST") {
                ...
                self.add_error(format!(
                    "expected FIRST or LAST after NULLS at {}",
                    self.peek_token.position
                ));
                return None;
            }
...
mod tests {
    ...
    #[test]
    fn test_parse_integer_i64_boundaries() {
        let mut max_parser = Parser::new("9223372036854775807");
        let max_expr = max_parser
            .parse_expression(Precedence::Lowest)
            .expect("i64::MAX should parse as an integer literal");
        assert!(max_parser.errors().is_empty());
        match max_expr {
            Expression::IntegerLiteral(lit) => assert_eq!(lit.value, i64::MAX),
            other => panic!("expected IntegerLiteral, got {other:?}"),
        }

        let mut min_parser = Parser::new("-9223372036854775808");
        let min_expr = min_parser
            .parse_expression(Precedence::Lowest)
            .expect("i64::MIN should parse through the unary-minus special case");
        assert!(min_parser.errors().is_empty());
        match min_expr {
            Expression::IntegerLiteral(lit) => assert_eq!(lit.value, i64::MIN),
            other => panic!("expected IntegerLiteral, got {other:?}"),
        }
    }
    ...
    fn test_parse_integer_overflow_is_an_error() {
        for literal in ["9223372036854775808", "-9223372036854775809"] {
            ...
            assert!(expression.is_none(), "{literal} must not produce an AST");
            assert_eq!(
                parser.errors().len(),
                1,
                "{literal} must produce exactly one parser error"
            );
            assert!(
                parser.errors()[0]
                    .message
                    .contains("is out of range for i64"),
                "unexpected parser error for {literal}: {}",
    ...
    #[test]
    fn test_expression_stops_before_statement_owned_alias() {
        let mut parser = Parser::new("1 AS answer");
        let expression = parser
            .parse_expression(Precedence::Lowest)
            .expect("integer expression should parse");

        assert!(matches!(expression, Expression::IntegerLiteral(_)));
        assert!(parser.peek_token_is_keyword("AS"));
    }
    ...
    fn test_parse_qualified_identifier() {
        ...
        match expr {
            Expression::QualifiedIdentifier(qi) => {
                ...
                assert!(qi.intermediate.is_none());
                assert_eq!(qi.name.value, "id");
                assert_eq!(qi.component_count(), 2);
            }
    ...
    #[test]
    fn test_parse_multi_part_identifier_path_preserves_components_and_positions() {
        let expr = parse_expr("m.sender_id.profile_id.display_name").unwrap();
        let Expression::QualifiedIdentifier(path) = expr else {
            panic!("expected unresolved multi-part QualifiedIdentifier");
        };
        ...
        let components: Vec<_> = path
            ...
            .map(|component| {
                (
                    ...
                    component.token.position.column,
                )
            })
            .collect();
        assert_eq!(
            ...
            vec![
                ("m".to_string(), 1),
                ("sender_id".to_string(), 3),
                ("profile_id".to_string(), 13),
                ("display_name".to_string(), 24),
            ]
        );
        assert_eq!(path.component_count(), 4);
        assert!(path.is_multi_part_path());
        assert_eq!(path.to_string(), "m.sender_id.profile_id.display_name");
    }

    #[test]
    fn test_parse_identifier_path_supports_eight_components_and_roundtrip() {
        let sql = "a.b.c.d.e.f.g.h";
        let expr = parse_expr(sql).unwrap();
        let Expression::QualifiedIdentifier(path) = &expr else {
            panic!("expected eight-component path");
        };
        assert_eq!(path.component_count(), 8);
        assert_eq!(expr.to_string(), sql);

        let reparsed = parse_expr(&expr.to_string()).unwrap();
        let Expression::QualifiedIdentifier(reparsed) = reparsed else {
            panic!("expected round-tripped path");
        };
        let values: Vec<_> = reparsed
            .components()
            .map(|component| component.value().to_string())
            .collect();
        assert_eq!(values, ["a", "b", "c", "d", "e", "f", "g", "h"]);
    }

    #[test]
    fn test_parse_identifier_path_preserves_quoted_and_keyword_components() {
        let expr = parse_expr("\"Root\".type.\"Target Field\"").unwrap();
        let Expression::QualifiedIdentifier(path) = expr else {
            panic!("expected quoted identifier path");
        };
        let values: Vec<_> = path
            .components()
            .map(|component| component.value().to_string())
            .collect();
        assert_eq!(values, ["Root", "type", "Target Field"]);
        assert_eq!(path.to_string(), "\"Root\".type.\"Target Field\"");
    }

    #[test]
    fn test_multi_part_qualified_star_is_rejected_without_breaking_table_star() {
        assert!(matches!(
            parse_expr("users.*").unwrap(),
            Expression::QualifiedStar(_)
        ));

        let mut parser = Parser::new("users.profile_id.*");
        assert!(parser.parse_expression(Precedence::Lowest).is_none());
        assert!(parser
            .errors()
            .iter()
            .any(|error| error.message.contains("qualified star cannot follow")));
    }
