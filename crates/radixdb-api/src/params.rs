...
pub use radixdb_core::ParamVec;

/// Exact public representation of a SQL DECIMAL value.
...
impl DecimalValue {
    /// Construct a validated DECIMAL payload.
    pub fn try_new(unscaled: i128, precision: u8, scale: u8) -> Result<Self> {
...
impl ToParam for Arc<str> {
    fn to_param(&self) -> Value {
        Value::Text(SmartString::from(Arc::clone(self)))
    }
}
...
impl ToParam for NaiveDate {
    fn to_param(&self) -> Value {
        ...
        Value::date(i32::try_from(days).expect("chrono date range fits SQL DATE domain"))
    }
}
...
impl ToParam for uuid::Uuid {
    fn to_param(&self) -> Value {
        Value::uuid(*self.as_bytes())
    }
}

impl ToParam for serde_json::Value {
    fn to_param(&self) -> Value {
        Value::try_json(self.to_string()).expect("serialized serde_json::Value is valid JSON")
    }
}

impl ToParam for DecimalValue {
    fn to_param(&self) -> Value {
        Value::try_decimal(self.unscaled, self.precision, self.scale)
            .expect("DecimalValue validates its invariant at construction")
    }
}
...
macro_rules! params {
    ...
    ($($param:expr),+ $(,)?) => {
        {
            let mut params = $crate::ParamVec::new();
            $(params.push($crate::ToParam::to_param(&$param));)+
            params
        }
    };
}
...
///
/// ```ignore
/// use radixdb::{Database, NamedParams, named_params};
///
/// let db = Database::open("memory://")?;
...
mod tests {
    ...
    #[test]
    fn test_arc_str_boundary_preserves_allocation() {
        let arc: Arc<str> = Arc::from("a heap string longer than the inline boundary");
        let ptr = arc.as_ptr();

        let smart = SmartString::from(Arc::clone(&arc));
        assert_eq!(smart.as_str().as_ptr(), ptr);
        assert_eq!(Arc::strong_count(&arc), 2);
        drop(smart);

        let value = Value::text_arc(Arc::clone(&arc));
        let Value::Text(text) = &value else {
            panic!("text_arc must produce Value::Text");
        };
        assert_eq!(text.as_str().as_ptr(), ptr);
        assert_eq!(Arc::strong_count(&arc), 2);
        drop(value);

        let param = arc.to_param();
        let Value::Text(text) = &param else {
            panic!("Arc<str>::to_param must produce Value::Text");
        };
        assert_eq!(text.as_str().as_ptr(), ptr);
        assert_eq!(Arc::strong_count(&arc), 2);
    }
