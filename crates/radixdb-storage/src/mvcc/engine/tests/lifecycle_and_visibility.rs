...
fn contended_point_update_does_not_hold_segment_maintenance_fence() {
    ...
    seed.get_table("items")
        .unwrap()
        .insert(Row::from_values(vec![Value::Integer(1), Value::Integer(0)]))
        .unwrap();
    seed.commit().unwrap();
...
fn uuid_primary_key_cold_updates_do_not_resurrect_predecessors() {
    ...
    for expected in 1..=8_i64 {
        ...
        assert_eq!(
            rows.len(),
            1,
            "duplicate before update {expected}: {rows:?}"
        );
        let row_id = rows[0].0;
...
fn uuid_primary_key_insert_batch_uses_one_bloom_pruned_cold_probe() {
    ...
    seed.get_table("items")
        ...
        .insert_batch(
            (0..4_096_u64)
                .map(|sequence| {
                    Row::from_values(vec![uuid_value(sequence), Value::Integer(sequence as i64)])
                })
                .collect(),
        )
        .unwrap();
    ...
    assert_eq!(
        posting.exact_calls, 1,
        "the batch must issue one cold lookup"
    );
...
fn composite_uuid_primary_key_insert_batch_uses_one_cold_probe() {
    ...
    let error = duplicate
        ...
        .insert_batch(vec![
            ...
            Row::from_values(vec![uuid_value(7 / 64), uuid_value(7), Value::Integer(7)]),
        ])
...
fn composite_uuid_primary_key_commit_recheck_batches_a_concurrent_seal() {
    ...
    let database_path = directory
        .path()
        .join("composite-uuid-cold-commit-recheck-batch");
    let mut config = Config::with_path(database_path.to_string_lossy().into_owned());
    config.persistence.checkpoint_interval = u32::MAX;
    config.persistence.checkpoint_on_close = false;
    config.persistence.target_volume_rows = 4_096;
...
fn concurrent_uuid_primary_key_cold_updates_keep_one_logical_row() {
        ...
        handles.push(std::thread::spawn(move || {
            ...
            if updated != 1 {
                ...
                return Err(Error::internal(format!(
                    "contended UUID update changed {updated} rows"
                )));
            }
            transaction.commit()?;
            Ok(true)
    ...
    assert_eq!(
        rows.len(),
        1,
        "UUID key must retain one logical owner: {rows:?}"
    );
    assert_eq!(rows[0].1.get(0), Some(&key));
