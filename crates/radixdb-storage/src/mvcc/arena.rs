...
impl RowArena {
    /// Create a new arena
    ///
    /// Keep the empty-table footprint bounded. Bulk/recovery owners that know
    /// cardinality must use `with_capacity`; eagerly reserving 10,000 rows for
    /// every catalog table multiplied retained memory by the table count.
    pub fn new() -> Self {
        Self {
    ...
    pub(crate) fn capacity(&self) -> usize {
        ...
        inner.data.capacity().min(inner.meta.capacity())
    }
    ...
    /// Drop all data and release the complete retained slot capacity.
    ///
    /// Callers must prove that no live version still references an arena slot.
    /// Ordinary partial cleanup continues to use `clear_batch` so later writes
    /// can reuse holes without growing the arena.
    pub fn clear_all(&self) {
        // Inserts acquire free_list before inner. Preserve that global order
        // here so a full seal cannot deadlock with a concurrent insert.
        let mut free_list = self.free_list.lock();
        let mut inner = self.inner.write();
        inner.data = Vec::new();
        inner.meta = Vec::new();
