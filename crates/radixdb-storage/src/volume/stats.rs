...
impl Default for ColumnAggregateStats {
    fn default() -> Self {
        Self {
            ...
            sum_float: 0.0,
            numeric_count: 0,
            min: Value::Null(radixdb_core::DataType::Null),
            max: Value::Null(radixdb_core::DataType::Null),
            non_null_count: 0,
        }
    }
}
...
mod tests {
    ...
    fn test_column_stats_with_nulls() {
        ...
        stats.accumulate(&Value::Null(radixdb_core::DataType::Float));
    ...
    #[test]
    fn float_nan_is_counted_as_persisted_numeric_metadata() {
        let mut stats = ColumnAggregateStats::default();
        let nan = Value::Float(f64::from_bits(0x7ff8_0000_0000_0042));
        stats.accumulate(&nan);

        assert_eq!(stats.numeric_count, 1);
        assert_eq!(stats.non_null_count, 1);
        assert!(stats.sum_float.is_nan());
        assert_eq!(stats.min, nan);
        assert_eq!(stats.max, nan);
    }
