...
/// One inclusive range over a core-owned physical index key. Planner plugins
/// never receive an index handle; this is the only declarative range boundary
/// consumed by storage.
...
pub trait Index: Send + Sync {
    ...
    /// Returns the canonical data owner when this object is a metadata-only
    /// wrapper. Implementations normally return `None`; wrappers use this hook
    /// so repeated metadata changes can replace, rather than nest, delegation.
    #[doc(hidden)]
    fn metadata_inner(&self) -> Option<std::sync::Arc<dyn Index>> {
        None
    }

    /// Returns the partial-index predicate, if this index only covers rows
    /// matching a CREATE INDEX ... WHERE predicate.
    fn partial_predicate(&self) -> Option<&crate::index::PartialIndexPredicate> {
        None
    }

    /// Returns the prepared semantic-to-physical key encoder owned by this
    /// runtime index, if any. The encoder itself has no storage authority.
    #[doc(hidden)]
    fn prepared_key_encoder(&self) -> Option<crate::index::PreparedIndexKeyEncoder> {
        None
    }

    /// Return every physical entry. Metadata wrappers use this conservative
    /// path when an unfamiliar predicate must remain a residual filter.
    #[doc(hidden)]
    ...
    /// Find a range whose bounds are already encoded in this index's physical
    /// key domain. Native indexes use their ordinary range contract; semantic
    /// encoding wrappers override this method to avoid encoding a second time.
    /// This remains a core-only boundary and is never exposed as a plugin ABI.
    #[doc(hidden)]
    ...
    /// Finds at most `limit` entries in index order inside a range. Concrete
    /// ordered indexes should override this method so ORDER BY/LIMIT does not
    /// first allocate the complete candidate set.
    ...
    /// Snapshot bytes for an HNSW graph. Other index types return `None`.
    fn hnsw_graph_bytes(&self) -> Result<Option<Vec<u8>>> {
        Ok(None)
    }

    /// Live row IDs already represented by an HNSW graph. Recovery uses this
    /// coverage identity to avoid rereading cold vector blocks.
