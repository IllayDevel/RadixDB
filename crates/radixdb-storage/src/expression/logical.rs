...
//! Logical expressions (AND, OR, NOT) for RadixDB
//!
    ...
    fn is_unknown_due_to_null(&self, row: &Row) -> bool {
        ...
        for expr in &self.expressions {
            match expr.evaluate(row) {
                Ok(true) => {}
                Ok(false) if expr.is_unknown_due_to_null(row) => saw_unknown = true,
...
mod tests {
    ...
    use radixdb_core::{DataType, Error, SchemaBuilder, Value};
    ...
    impl Expression for FailingExpr {
        fn evaluate(&self, _row: &Row) -> Result<bool> {
            Err(Error::expression_evaluation("forced predicate failure"))
        }
    ...
    #[test]
    fn test_or_propagates_error_after_false_operand() {
        let schema = test_schema();
        let row = test_row();
        ...
        let error = expr.evaluate(&row).unwrap_err().to_string();
        assert!(
            error.contains("forced predicate failure"),
            "unexpected OR error: {error}"
        );
    }

    #[test]
    fn test_or_keeps_short_circuit_before_error() {
        let schema = test_schema();
        let row = test_row();
