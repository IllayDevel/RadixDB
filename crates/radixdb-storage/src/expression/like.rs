...
impl Expression for LikeExpr {
    ...
    fn get_like_prefix_info(&self) -> Option<(&str, String, bool)> {
        ...
        // Extract the same literal prefix recognized by compile_pattern.
        ...
        while let Some(ch) = chars.next() {
            match ch {
                ...
                '\\' => match chars.peek().copied() {
                    Some('%' | '_' | '\\') => {
                        prefix.push(chars.next().expect("peeked LIKE escape"));
                    }
                    _ => prefix.push('\\'),
                },
    ...
    fn is_unknown_due_to_null(&self, row: &Row) -> bool {
        self.col_index
            ...
            .is_some_and(radixdb_core::Value::is_null)
    }
...
mod tests {
    ...
    #[test]
    fn escaped_wildcards_produce_literal_prefixes() {
        let expr = LikeExpr::new("name", r"hello\%world%");
        let (_, prefix, _) = expr.get_like_prefix_info().expect("literal prefix");
        assert_eq!(prefix, "hello%world");

        let escaped_slash = LikeExpr::new("name", r"root\\path_");
        let (_, prefix, _) = escaped_slash
            .get_like_prefix_info()
            .expect("escaped slash prefix");
        assert_eq!(prefix, r"root\path");
    }

    #[test]
    fn like_reports_null_as_unknown() {
        let schema = test_schema();
        let mut expr = LikeExpr::new("name", "John%");
        expr.prepare_for_schema(&schema);
        let row = Row::from(vec![
