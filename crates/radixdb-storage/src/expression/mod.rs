...
//! Compiled storage expression system for RadixDB.
//!
...
pub use function::{EvalExpr, FunctionArg, FunctionExpr, ScalarEvaluator};
...
pub trait Expression: Send + Sync + Debug {
    ...
    /// Extract exact BETWEEN metadata if this expression is a BETWEEN predicate.
    ///
    /// This is intentionally separate from `collect_comparisons()`: BETWEEN can
    /// expose comparison bounds for pruning, but callers that want to skip
    /// row-level filter evaluation need to know they are handling the exact
    /// predicate semantics, not just a pruning approximation.
    fn get_between_info(&self) -> Option<(&str, &Value, &Value, bool, bool)> {
        None
    }

    /// Extract exact range metadata if this expression is a custom range
    /// predicate (`>=/> lower AND <=/< upper`).
    fn get_range_info(&self) -> Option<(&str, &Value, &Value, bool, bool)> {
        None
    }

    /// Extract exact IN-list metadata if this expression is an IN/NOT IN
    /// predicate.
    fn get_in_list_info(&self) -> Option<(&str, &[Value], bool, bool)> {
        None
    }

    /// Extract exact NULL-check metadata if this expression is IS NULL /
    /// IS NOT NULL.
    fn get_null_check_info(&self) -> Option<(&str, bool)> {
        None
    }
    ...
    /// Collect exact IN-list predicates from an AND tree.
    ///
    /// OR/NOT are intentionally not traversed because an IN predicate inside a
    /// disjunction is not a safe standalone rejection filter.
    fn collect_in_list_infos(&self) -> Vec<(&str, &[Value], bool, bool)> {
        if let Some(info) = self.get_in_list_info() {
            vec![info]
        } else if let Some(children) = self.get_and_operands() {
            let mut result = Vec::new();
            for child in children {
                result.extend(child.collect_in_list_infos());
            }
            result
        } else {
            vec![]
    ...
    /// Collect exact NULL-check predicates from an AND tree.
    fn collect_null_check_infos(&self) -> Vec<(&str, bool)> {
        if let Some(info) = self.get_null_check_info() {
            vec![info]
        } else if let Some(children) = self.get_and_operands() {
            let mut result = Vec::new();
            for child in children {
                result.extend(child.collect_null_check_infos());
            }
            result
        } else {
            vec![]
    ...
    /// Collect LIKE prefix predicates from an AND tree.
    ///
    /// The prefix is a necessary condition, not the full LIKE semantics. This
    /// is safe for non-negated LIKE rejection (`not starts_with(prefix)`), but
    /// callers must not use it as proof that the whole filter passed.
    fn collect_like_prefix_infos(&self) -> Vec<(&str, String, bool)> {
        if let Some(info) = self.get_like_prefix_info() {
            vec![info]
        } else if let Some(children) = self.get_and_operands() {
            let mut result = Vec::new();
            for child in children {
                result.extend(child.collect_like_prefix_infos());
            }
            result
        } else {
            vec![]
