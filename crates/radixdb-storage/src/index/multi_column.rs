...
pub struct MultiColumnIndex {
    ...
    /// Publishes the hash, ordered, prefix, and reverse representations as one
    /// logical index state. Readers never observe a partially applied mutation.
...
impl Index for MultiColumnIndex {
    ...
    fn add(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        // Determine the old key, but keep it published until uniqueness of the
    ...
    fn find_range(
        ...
        let end = if max.is_empty() || max_is_partial {
            // A partial upper key such as [timestamp] sorts before every real
            // [timestamp, id] key. Using it as the BTree end bound would drop
            // the complete inclusive prefix. Iterate from the lower bound and
            // stop lexicographically in the loop instead.
            Bound::Unbounded
        ...
        for (key, row_ids) in sorted_values.range((start, end)) {
            ...
            // Check min bounds (only for partial key prefix). Slice ordering is
            // the same lexicographic ordering as CompositeKey; comparing every
            // component independently would reject valid keys such as
            // [4, 100] under upper bound [5, 10].
...
mod tests {
    ...
    fn v2_r3_multi_authorities_publish_and_reject_stale_remove_together() {
        ...
        // Materialize every lazy authority before mutation.
