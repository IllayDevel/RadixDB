...
use radixdb_core::{CompactArc, I64Map};
...
pub struct BitmapIndex {
    ...
    /// A normal signed-key map is required here: `I64Map` reserves i64::MIN,
    /// while public row identity covers the complete i64 domain.
...
impl BitmapIndex {
    ...
    pub fn new(
        ...
        .expect("internal BitmapIndex metadata must be coherent")
    }
    ...
    pub fn try_new(
        ...
        if column_names.len() != 1 {
            return Err(Error::invalid_argument(
                "BitmapIndex supports exactly one structural Value column",
            ));
        }
        Ok(Self {
            name,
            ...
            row_to_value: RwLock::new(AHashMap::with_capacity(expected_rows)),
    ...
    fn add_multi_column_slow(
        ...
        mut row_to_value: parking_lot::RwLockWriteGuard<'_, AHashMap<i64, CompactArc<Value>>>,
    ) -> Result<()> {
...
impl Index for BitmapIndex {
    ...
    fn add(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        // `as u64` is a reversible bit-pattern mapping for the full i64
        // domain; result paths cast it back to i64.
    ...
    fn remove(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        if values.len() != 1 {
            return Err(Error::invalid_argument("bitmap index requires one value"));
        }
        let row_id_u64 = row_id as u64;
        ...
        let Some(arc_key) = row_to_value.get(&row_id).cloned() else {
            return Ok(());
        };
        if arc_key.as_ref() != &values[0] {
            return Err(Error::invalid_argument(format!(
                "stale value supplied while removing row {row_id} from index '{}'",
                self.name
            )));
        }
    ...
    fn remove_batch_slice(&self, entries: &[(i64, &[Value])]) -> Result<()> {
        ...
        for &(row_id, values) in entries {
            ...
            removals.push((row_id, arc_key));
        }

        for (row_id, arc_key) in removals {
            let row_id_u64 = row_id as u64;
    ...
    fn get_row_ids_equal_into(&self, values: &[Value], buffer: &mut Vec<i64>) -> Result<()> {
        ...
        if values.len() != self.column_ids.len() {
            return Err(Error::invalid_argument(
                "bitmap index requires exact values for every indexed column",
            ));
        }
    ...
    fn get_row_ids_in_range_into(
        ...
        Err(Error::NotSupported(
            "bitmap index does not support range queries".to_string(),
        ))
    }
    ...
    fn get_filtered_row_ids(&self, expr: &dyn Expression) -> Result<RowIdVec> {
        ...
        Ok(RowIdVec::from_vec(collected))
    }
    ...
    fn clear(&self) {
        *self.bitmaps.write() = AHashMap::default();
        *self.row_to_value.write() = AHashMap::default();
...
mod tests {
    ...
    #[test]
    fn test_bitmap_index_supports_full_signed_row_id_domain() {
        let index = BitmapIndex::new(
            "idx_status".to_string(),
            "orders".to_string(),
            vec!["status".to_string()],
        ...
        let key = [Value::Text("pending".into())];
        index.add(&key, -1, 0).unwrap();
        index.add(&key, i64::MIN, 0).unwrap();
        let mut row_ids = index.get_row_ids_equal(&key).unwrap().into_vec();
        row_ids.sort_unstable();
        assert_eq!(row_ids, vec![i64::MIN, -1]);

        index.remove(&key, -1, 0).unwrap();
        index.remove(&key, i64::MIN, 0).unwrap();
        assert!(index.get_row_ids_equal(&key).unwrap().is_empty());
    }

    #[test]
    fn v2_r3_stale_remove_is_rejected_without_losing_bitmap_ownership() {
        let index = BitmapIndex::new(
            "idx".into(),
            "items".into(),
            vec!["state".into()],
