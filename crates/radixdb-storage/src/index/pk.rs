...
impl PkIndex {
    ...
    /// Extract the exact canonical INTEGER identity from the first value.
    ///
    /// SQL numeric equality spans INTEGER, FLOAT and DECIMAL. PkIndex stores
    /// physical INTEGER row IDs, so it may probe another numeric domain only
    /// when that value has exactly the same canonical INTEGER identity.
    #[inline]
    fn extract_i64(values: &[Value]) -> Option<i64> {
        values.first().and_then(Value::exact_integer_identity)
    }
...
impl Index for PkIndex {
    ...
    fn get_row_ids_in_into(&self, value_list: &[Value], buffer: &mut Vec<i64>) -> Result<()> {
        ...
        for value in value_list {
            if let Some(row_id) = value.exact_integer_identity() {
                if inner.contains(row_id) {
                    buffer.push(row_id);
                }
            }
    ...
    fn for_each_group(
        ...
        if inner.overflow_empty() {
            ...
            let mut err: Option<radixdb_core::Error> = None;
            for_each_set_bit(&inner.words, |rid| {
    ...
    fn close(&mut self) -> Result<()> {
        Err(Error::NotSupported(
            "primary-key index lifecycle is owned by its table".to_string(),
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_pk_index_uses_exact_cross_domain_numeric_identity() {
        let idx = make_idx();
        let boundary = 1_i64 << 53;
        for id in [i64::MIN, boundary, boundary + 1, i64::MAX] {
            idx.add(&[Value::Integer(id)], id, id).unwrap();
        }

        assert_eq!(
            idx.find(&[Value::Float(boundary as f64)]).unwrap(),
            vec![IndexEntry {
                row_id: boundary,
                ref_id: boundary,
            }]
        ...
        assert_eq!(
            idx.find(&[Value::Float((boundary + 1) as f64)]).unwrap(),
            vec![IndexEntry {
                row_id: boundary,
                ref_id: boundary,
            }],
            "the rounded FLOAT identity must not select the adjacent INTEGER"
        );
        assert_eq!(
            idx.find(&[Value::Float(i64::MIN as f64)]).unwrap()[0].row_id,
            i64::MIN
        );
        assert!(idx.find(&[Value::Float(0.5)]).unwrap().is_empty());
        assert!(idx
            .find(&[Value::Float(i64::MAX as f64)])
            .unwrap()
            .is_empty());

        let exact_decimal = Value::decimal(boundary as i128 * 10, 17, 1);
        assert_eq!(idx.find(&[exact_decimal]).unwrap()[0].row_id, boundary);
        assert!(idx.find(&[Value::decimal(5, 1, 1)]).unwrap().is_empty());

        let mut in_rows = Vec::new();
        idx.get_row_ids_in_into(
            &[
                Value::Float(boundary as f64),
                Value::decimal((boundary + 1) as i128, 16, 0),
                Value::Float(0.5),
            ],
            &mut in_rows,
        )
        .unwrap();
        assert_eq!(in_rows, vec![boundary, boundary + 1]);
    }
    ...
    #[test]
    fn v2_r3_pk_close_declines_table_owned_lifecycle() {
        let mut idx = make_idx();
        idx.add(&[Value::Integer(1)], 1, 1).unwrap();
        assert!(idx.close().is_err());
        assert_eq!(idx.find(&[Value::Integer(1)]).unwrap()[0].row_id, 1);
    }
}
