...
impl Index for BTreeIndex {
    ...
    fn add(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        // Keep its old posting intact until the new unique key is admitted.
        let old_arc = if let Some(old_arc) = row_to_value.get(row_id) {
        ...
        // Check uniqueness before mutating either forward or reverse map.
    ...
    fn find_range(
        ...
        let min_val = &min[0];
        let max_val = &max[0];
        match min_val.cmp(max_val) {
            std::cmp::Ordering::Greater => return Ok(Vec::new()),
            std::cmp::Ordering::Equal if !min_inclusive || !max_inclusive => {
                return Ok(Vec::new());
            }
            _ => {}
        }
    ...
    fn get_row_ids_equal_into(&self, values: &[Value], buffer: &mut Vec<i64>) -> Result<()> {
        self.check_closed()?;
        if values.is_empty() {
            return Err(Error::invalid_argument("btree equality key is empty"));
        }
    ...
    fn get_row_ids_in_range_into(
        ...
        if let (Some(min_val), Some(max_val)) = (min_value.first(), max_value.first()) {
            match min_val.cmp(max_val) {
                std::cmp::Ordering::Greater => return Ok(()),
                std::cmp::Ordering::Equal if !include_min || !include_max => return Ok(()),
                _ => {}
            }
        }
        ...
        let min_bound = match min_value.first() {
            None => Bound::Unbounded,
            Some(value) if include_min => Bound::Included(CompactArc::new(value.clone())),
            Some(value) => Bound::Excluded(CompactArc::new(value.clone())),
        };
        let max_bound = match max_value.first() {
            None => Bound::Unbounded,
            Some(value) if include_max => Bound::Included(CompactArc::new(value.clone())),
            Some(value) => Bound::Excluded(CompactArc::new(value.clone())),
        };
    ...
    fn get_filtered_row_ids(&self, expr: &dyn Expression) -> Result<RowIdVec> {
        ...
        // Compound expressions are evaluated against every indexed value. This
        // avoids maintaining a second, subtly different bound normalizer here;
        // ordinary table planning owns optimized conjunction/range admission.
        ...
        if row_to_value.len() >= PARALLEL_FILTER_THRESHOLD {
            ...
            let evaluated: Result<Vec<Option<i64>>> = pairs
                .par_iter()
                .map(|pair| {
                    let (row_id, arc_value) = *pair;
                    // Dereference Arc to get the Value
                    let row = radixdb_core::Row::from_values(vec![(**arc_value).clone()]);
...
mod tests {
    ...
    impl Expression for FailingExpr {
        fn evaluate(&self, _row: &Row) -> Result<bool> {
            Err(Error::expression_evaluation(
                "forced index predicate failure",
            ))
        }
    ...
    #[test]
    fn test_btree_unique_mixed_numeric_identity_at_f64_boundary() {
        let index = BTreeIndex::new(
            "idx_unique_numeric_boundary".to_string(),
            "test".to_string(),
            0,
            "value".to_string(),
            DataType::Float,
        ...
        index.add(&[Value::Integer(boundary)], 1, 1).unwrap();
        assert_eq!(
            index
                .find(&[Value::Float(boundary as f64)])
                .unwrap()
                .iter()
        ...
        assert!(index.add(&[Value::Float(boundary as f64)], 2, 2).is_err());

        index.add(&[Value::Integer(boundary + 1)], 3, 3).unwrap();
        assert_eq!(
            index
                .find(&[Value::Integer(boundary + 1)])
                .unwrap()
                .iter()
                .map(|entry| entry.row_id)
                .collect::<Vec<_>>(),
        ...
        index.add(&[Value::Integer(-boundary)], 4, 4).unwrap();
        assert!(index
            .add(&[Value::Float(-(boundary as f64))], 5, 5)
            .is_err());
        index.add(&[Value::Integer(-boundary - 1)], 6, 6).unwrap();
        assert_eq!(
            index
                .find(&[Value::Integer(-boundary - 1)])
                .unwrap()
    ...
    #[test]
    fn test_btree_unique_batch_mixed_numeric_identity_at_f64_boundary() {
        let index = BTreeIndex::new(
            "idx_unique_numeric_boundary_batch".to_string(),
            "test".to_string(),
            0,
            "value".to_string(),
            DataType::Float,
        ...
        let boundary = 1_i64 << 53;
        let exact_integer = [Value::Integer(boundary)];
        let exact_float = [Value::Float(boundary as f64)];

        assert!(index
            .add_batch_slice(&[(1, exact_integer.as_slice()), (2, exact_float.as_slice()),])
            .is_err());
        assert_eq!(index.entry_count(), 0);

        let distinct_neighbor = [Value::Integer(boundary + 1)];
        let negative_exact = [Value::Integer(-boundary)];
        index
            .add_batch_slice(&[
                (1, exact_integer.as_slice()),
                (3, distinct_neighbor.as_slice()),
                (4, negative_exact.as_slice()),
            ])
            .unwrap();
    ...
    fn test_btree_index_row_id_helpers() {
        ...
        // get_row_ids_equal
        let row_ids = index.get_row_ids_equal(&[Value::Integer(100)]).unwrap();
        assert_eq!(row_ids.len(), 2);

        // get_row_ids_in_range
        let row_ids = index
            .get_row_ids_in_range(&[Value::Integer(100)], &[Value::Integer(200)], true, true)
            .unwrap();
    ...
    #[test]
    fn v2_r3_btree_contradictory_ranges_are_empty_without_panicking() {
        let index = create_test_index();
        index.add(&[Value::Integer(5)], 1, 1).unwrap();

        assert!(index
            .find_range(&[Value::Integer(10)], &[Value::Integer(1)], true, true)
            .unwrap()
            .is_empty());
        assert!(index
            .find_range(&[Value::Integer(5)], &[Value::Integer(5)], false, true)
            .unwrap()
            .is_empty());
        assert!(index
            .get_row_ids_in_range(&[Value::Integer(10)], &[Value::Integer(1)], true, true,)
            .unwrap()
            .is_empty());
    }

    #[test]
    fn v2_r3_btree_filtered_lookup_is_complete_and_fallible() {
        let mut index = create_test_index();
        for value in [10, 20, 30] {
            index.add(&[Value::Integer(value)], value, value).unwrap();
        }

        let greater = ComparisonExpr::gt("id", Value::Integer(10));
        let mut rows = index.get_filtered_row_ids(&greater).unwrap().into_vec();
        rows.sort_unstable();
        assert_eq!(rows, vec![20, 30]);
        assert!(index.get_filtered_row_ids(&FailingExpr).is_err());

        index.close().unwrap();
        assert!(index.get_row_ids_equal(&[Value::Integer(10)]).is_err());
        assert!(index.get_filtered_row_ids(&greater).is_err());
    }
}
