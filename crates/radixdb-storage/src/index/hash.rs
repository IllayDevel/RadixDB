...
//! ## Known Limitations:
//!
//! ### Lock Pattern for Write Operations
//! The `add` and `remove` methods acquire two RwLock write locks simultaneously:
...
use crate::expression::{ComparisonExpr, Expression, InListExpr};
...
impl HashIndex {
    ...
    pub fn new(
        ...
        .expect("internal HashIndex metadata must be coherent")
    }
...
impl Index for HashIndex {
    ...
    fn add(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        // Check whether this is a no-op, but do not remove the old posting
        // until the new unique key has been admitted.
        let mut old_hash_to_remove = None;
        if let Some(&old_hash) = row_to_hash.get(row_id) {
            ...
            old_hash_to_remove = Some(old_hash);
        }
    ...
    fn remove(&self, values: &[Value], row_id: i64, _ref_id: i64) -> Result<()> {
        ...
        let Some(&hash) = row_to_hash.get(row_id) else {
            return Ok(());
        };
        let stored_matches = hash_to_values.get(&hash).is_some_and(|entries| {
    ...
    fn remove_batch_slice(&self, entries: &[(i64, &[Value])]) -> Result<()> {
        ...
        for &(row_id, values) in entries {
            ...
            let stored_matches = hash_to_values.get(&hash).is_some_and(|value_entries| {
                value_entries.iter().any(|(stored_values, row_ids)| {
                    row_ids.contains(&row_id) && Self::values_match(stored_values, values)
                })
            });
            if !stored_matches {
            ...
            removals.push((row_id, hash));
        }

        for (row_id, hash) in removals {
            row_to_hash.remove(row_id);
    ...
    fn get_row_ids_equal_into(&self, values: &[Value], buffer: &mut Vec<i64>) -> Result<()> {
        ...
        if values.len() != self.column_ids.len() {
            return Err(Error::invalid_argument(
                "hash index requires exact values for every indexed column",
            ));
        }
    ...
    fn get_row_ids_in_range_into(
        ...
        Err(Error::NotSupported(
            "hash index does not support range queries".to_string(),
        ))
    }
    ...
    fn clear(&self) {
        *self.row_to_hash.write() = I64Map::new();
        *self.hash_to_values.write() = FxHashMap::default();
    }
...
mod tests {
    ...
    #[test]
    fn test_unique_hash_index_uses_exact_integer_float_identity() {
        let index = HashIndex::new(
            "idx_number_unique".to_string(),
            "numbers".to_string(),
        ...
        index.add(&[Value::Integer(boundary)], 1, 0).unwrap();

        // The exactly representable Float is the same logical key.
        assert!(index.add(&[Value::Float(boundary as f64)], 2, 0).is_err());
        assert_eq!(
            index.find(&[Value::Float(boundary as f64)]).unwrap(),
            vec![IndexEntry {
                row_id: 1,
                ref_id: 0
            }]
        ...
        // 2^53 + 1 is a distinct exact Integer even though converting it to
        // f64 rounds back to 2^53.
        index.add(&[Value::Integer(boundary + 1)], 2, 0).unwrap();
        assert_eq!(
            index.find(&[Value::Integer(boundary + 1)]).unwrap(),
            vec![IndexEntry {
                row_id: 2,
                ref_id: 0
            }]
        ...
        assert_eq!(
            index.find(&[Value::Float((boundary + 1) as f64)]).unwrap(),
            vec![IndexEntry {
                row_id: 1,
                ref_id: 0
            }]
        ...
        let negative_boundary = -boundary;
        index
            .add(&[Value::Float(negative_boundary as f64)], 3, 0)
            .unwrap();
        assert!(index
            .add(&[Value::Integer(negative_boundary)], 4, 0)
            .is_err());
        index
            .add(&[Value::Integer(negative_boundary - 1)], 4, 0)
            .unwrap();
        assert_eq!(
            index
                .find(&[Value::Float((negative_boundary - 1) as f64)])
                .unwrap(),
            vec![IndexEntry {
                row_id: 3,
                ref_id: 0
        ...
        assert_eq!(
            index
                .find(&[Value::Integer(negative_boundary - 1)])
                .unwrap(),
            vec![IndexEntry {
                row_id: 4,
                ref_id: 0
    ...
    #[test]
    fn test_unique_hash_index_batch_uses_exact_integer_float_identity() {
        let index = HashIndex::new(
            "idx_number_unique".to_string(),
            "numbers".to_string(),
            vec!["number".to_string()],
        ...
        let duplicate_values = [
            ...
            vec![Value::Float(boundary as f64)],
        ];
        let duplicate_entries = [
            (1, duplicate_values[0].as_slice()),
            (2, duplicate_values[1].as_slice()),
        ];
        assert!(index.add_batch_slice(&duplicate_entries).is_err());
        assert!(index.find(&[Value::Integer(boundary)]).unwrap().is_empty());

        let distinct_values = [
            vec![Value::Integer(boundary)],
            vec![Value::Integer(boundary + 1)],
        ];
        let distinct_entries = [
            (1, distinct_values[0].as_slice()),
            (2, distinct_values[1].as_slice()),
        ];
        index.add_batch_slice(&distinct_entries).unwrap();
    ...
    #[test]
    fn test_hash_index_batch_update_remove_and_clear() {
        let index = HashIndex::new(
            "idx_status".to_string(),
            "orders".to_string(),
            vec!["status".to_string()],
        ...
        let initial = [
            (1, vec![Value::Text("pending".into())]),
            (2, vec![Value::Text("pending".into())]),
            (3, vec![Value::Text("shipped".into())]),
        ];
        let initial_refs: Vec<_> = initial
            .iter()
            .map(|(row_id, values)| (*row_id, values.as_slice()))
            .collect();
        index.add_batch_slice(&initial_refs).unwrap();

        let pending = index.find(&[Value::Text("pending".into())]).unwrap();
        assert_eq!(
            pending.iter().map(|entry| entry.row_id).collect::<Vec<_>>(),
            [1, 2]
        );

        let update = [(1, vec![Value::Text("delivered".into())])];
        let update_refs: Vec<_> = update
            .iter()
            .map(|(row_id, values)| (*row_id, values.as_slice()))
            .collect();
        index.add_batch_slice(&update_refs).unwrap();

        let remove = [(2, vec![Value::Text("pending".into())])];
        let remove_refs: Vec<_> = remove
            .iter()
            .map(|(row_id, values)| (*row_id, values.as_slice()))
            .collect();
        index.remove_batch_slice(&remove_refs).unwrap();

        assert!(index
            .find(&[Value::Text("pending".into())])
            .unwrap()
            .is_empty());
        assert_eq!(
            index.find(&[Value::Text("delivered".into())]).unwrap()[0].row_id,
            1
        );

        index.clear();
        assert!(index
            .find(&[Value::Text("delivered".into())])
            .unwrap()
            .is_empty());
        assert!(index
            .find(&[Value::Text("shipped".into())])
            .unwrap()
            .is_empty());
    }

    #[test]
    fn test_hash_index_concurrent_non_unique_same_value_keeps_all_rows() {
        ...
        let handles: Vec<_> = (0..WRITERS)
            .map(|writer| {
                ...
                thread::spawn(move || {
                    barrier.wait();
                    for offset in 0..ROWS_PER_WRITER {
                        let row_id = (writer * ROWS_PER_WRITER + offset + 1) as i64;
                        index
                            .add(&[Value::Text("pending".into())], row_id, 0)
                            .unwrap();
                    }
                })
            })
        ...
        let actual: Vec<_> = index
            ...
            .collect();
        let expected: Vec<_> = (1..=(WRITERS * ROWS_PER_WRITER) as i64).collect();
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_hash_index_concurrent_unique_same_value_has_one_winner() {
        const WRITERS: usize = 8;
        ...
        let handles: Vec<_> = (0..WRITERS)
            .map(|writer| {
                ...
                thread::spawn(move || {
                    barrier.wait();
                    let row_id = writer as i64 + 1;
                    (
                        row_id,
                        index.add(&[Value::Text("same@example.com".into())], row_id, 0),
                    )
                })
            })
            .collect();
        ...
        let winners: Vec<_> = outcomes
            .iter()
            .filter_map(|(row_id, result)| result.is_ok().then_some(*row_id))
            .collect();
        assert_eq!(winners.len(), 1);

        let rows = index
            .find(&[Value::Text("same@example.com".into())])
            .unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].row_id, winners[0]);
    }

    #[test]
    fn v2_r3_stale_remove_is_rejected_without_losing_hash_ownership() {
        let index = HashIndex::new(
            "idx".into(),
            "items".into(),
            vec!["value".into()],
        ...
        index.add(&c, 2, 2).unwrap();

        assert!(index.remove(&b, 1, 1).is_err());
        assert_eq!(index.find(&a).unwrap()[0].row_id, 1);

        let removals = [(1, a.as_slice()), (2, b.as_slice())];
        assert!(index.remove_batch_slice(&removals).is_err());
        assert_eq!(index.find(&a).unwrap()[0].row_id, 1);
        assert_eq!(index.find(&c).unwrap()[0].row_id, 2);

        index.add(&b, 1, 1).unwrap();
        assert!(index.find(&a).unwrap().is_empty());
        assert_eq!(index.find(&b).unwrap()[0].row_id, 1);
    }
}
