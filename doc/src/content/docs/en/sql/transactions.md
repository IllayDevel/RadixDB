...
1.2.25 contract, including the differences between `READ COMMITTED` and `SNAPSHOT`.
...
The 1.2.25 SQL executor accepts only `READ COMMITTED` and `SNAPSHOT`. The following
...
The 1.2.25 CLI preserves `READ COMMITTED` and `SNAPSHOT` clauses on `BEGIN` and
