...
The RadixDB 1.2.25 extension SDK lets an author write ordinary typed Rust while
...
reserved and deliberately fail compilation in 1.2.25. Native aggregate, window
...
The host publishes no partial batch after an error. The 1.2.25 scalar callback
...
An operator is metadata over an already declared native scalar. The 1.2.25 Rust
...
classes are not available in the 1.2.25 SDK.
...
The 1.2.25 SDK does not generate text input/output callbacks for SQL literals and
