...
The RadixDB 1.2.21 extension SDK lets an author write ordinary typed Rust while
...
reserved and deliberately fail compilation in 1.2.21. Native aggregate, window
...
The host publishes no partial batch after an error. The 1.2.21 scalar callback
...
An operator is metadata over an already declared native scalar. The 1.2.21 Rust
...
classes are not available in the 1.2.21 SDK.
...
The 1.2.21 SDK does not generate text input/output callbacks for SQL literals and
