...
`IGNORE MISSING` are not part of the 1.2.4 grammar. `IF NOT EXISTS` succeeds only
...
`ALTER EXTENSION UPDATE` in 1.2.4.
...
The 1.2.4 Rust SDK does not publish generic SQL text input/output callbacks.
...
not accepted in 1.2.4. `LANGUAGE RADIX` routines are a separate facility.
...
`>`; hash and bitmap classes require `=`. The Rust 1.2.4 authoring SDK rejects
