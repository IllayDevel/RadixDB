...
`IGNORE MISSING` are not part of the 1.2.19 grammar. `IF NOT EXISTS` succeeds only
...
`ALTER EXTENSION UPDATE` in 1.2.19.
...
The 1.2.19 Rust SDK does not publish generic SQL text input/output callbacks.
...
not accepted in 1.2.19. `LANGUAGE RADIX` routines are a separate facility.
...
`>`; hash and bitmap classes require `=`. The Rust 1.2.19 authoring SDK rejects
