...
Руководство 1.2.25 соответствует выпущенному снимку исходников 1.2.25 и wire
protocol 18. Согласованные crates workspace и бинарники сообщают Cargo package
version 1.2.25. Связывайте бинарник с точной ревизией исходников и lockfile по
полной build identity.

Этот технический выпуск сериализует автоматическое освобождение неизменяемых
компонентов с владением physical restore и compaction. Поздний цикл очистки
больше не может создать quarantine во время замены физического корня базы.
Регрессионные проверки отмены, конкурентного изменения схемы и file-backed
checkpoint получили детерминированные и ограниченные контракты выполнения.

SQL, wire protocol, storage format, конфигурация и публичные API не изменились
относительно 1.2.21.

## 1.2.21 - 2026-09-14

Руководство 1.2.21 соответствует выпущенному снимку исходников 1.2.21 и wire
protocol 18. Согласованные crates workspace и бинарники сообщают Cargo package
version 1.2.21. Связывайте бинарник с точной ревизией исходников и lockfile по
полной build identity.

Этот технический выпуск усиливает чувствительные к таймингам CI-проверки отмены
TCP-запроса при разрыве соединения и финализации долговечного Job scheduler. По
сравнению с 1.2.19 не изменились runtime-поведение, SQL, wire protocol, storage
format, конфигурация и публичные API. Опубликованные crates получили отдельные
тематические keywords для движка, хранилища, клиента, ORM, application SDK,
procedural runtime и пакетов расширений.

## 1.2.19 - 2026-09-14

Руководство 1.2.19 соответствует выпущенному снимку исходников 1.2.19 и wire
protocol 18. Согласованные crates workspace и бинарники сообщают Cargo package
version 1.2.19. Связывайте бинарник с точной ревизией исходников и lockfile по
полной build identity.
...
Предметно-независимый `radixdb-app-sdk` добавляет слой доверенного прикладного
сервиса над асинхронным клиентом и ORM. Он предоставляет ограниченные request и
session identity, deadlines, кооперативную отмену, лимиты типизированных
результатов, устойчивую классификацию повторов и исходов, fingerprints схемы,
сгенерированные контракты таблиц и процедур и versioned application events.
Аутентификация продукта, авторизация, маршруты и бизнес-правила остаются вне
репозитория базы. Сквозной пример приведён в главе
[Application SDK](../../clients/application-sdk/).

### Типы SQL и ограничения

- `TEXT(n)` ограничивает длину текста; `VARCHAR(n)` и `CHAR(n)` принимаются
  как варианты записи ограниченного текстового типа.
- `DOUBLE PRECISION` сохраняет самостоятельную идентичность в SQL и каталоге,
  используя представление f64.
- `TIMESTAMP` представляет календарные дату и время без неявного преобразования
  через часовой пояс соединения.
- `TIME` представляет время суток независимо от конкретного момента времени.
- `TIMESTAMPTZ` представляет момент времени. Часовой пояс IANA или фиксированное
  смещение соединения используются при явных преобразованиях календарного
  времени и момента; неоднозначное и несуществующее время при переходах
  летнего времени отклоняется.
- Производные сложение, вычитание и умножение `DECIMAL` сохраняют точность при
  разных масштабах, с проверками точности и переполнения.

`SET TIME ZONE` и `SHOW TIME ZONE` управляют часовым поясом соединения.
Скалярные и упорядоченные составные первичные ключи поддерживают нецелочисленные
значения и обеспечивают уникальность полного ключа; `NULL` запрещён в каждом
компоненте. Ограничения, добавляемые после загрузки данных, принимают
квалифицированные имена отношений и отклоняют конфликтующие определения.
Проверки уникальности в холодном хранилище выполняются пакетно при массовой
вставке и перепроверке commit, включая составные ключи. Группы строк переменного
размера разделяются по байтовому лимиту до публикации.

Привилегия `DESCRIBE` уровня базы управляет получением схемы приложениями.
Сгенерированные прикладные контракты сохраняют детерминированные fingerprints
и типизированные аргументы/результаты процедур; процедуры с табличным
результатом сохраняют число возвращаемых строк.

### Trusted native extensions

Версия 1.2.19 добавляет stable C ABI 1.0, безопасный Rust authoring SDK и
...
с catalog 6.2, а protocol 18 сохраняет external type identity и codec revision
на wire.
...
Поддерживаются целевые платформы пакетов `x86_64-unknown-linux-gnu` и
`aarch64-unknown-linux-gnu`. Target пакета и архитектура ELF должны совпадать
с сервером; пакет для другой архитектуры отклоняется.
...
Protocol 18 не совместим по wire с protocol 14. Обновляйте server и client как
...
описанную ниже. Ограничения раздела относятся к 1.1, а не к выпускам 1.2.25 и
1.2.21 выше.
