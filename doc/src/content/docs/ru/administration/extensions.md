...
RadixDB 1.2.19 загружает доверенные пакеты нативных расширений при запуске
...
В RadixDB 1.2.19 нет hot reload и команды обновления на месте
...
`CASCADE` для объектов расширения в 1.2.19 не поддерживается. `RESTRICT` запрещает
