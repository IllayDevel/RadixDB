...
RadixDB 1.2.25 загружает доверенные пакеты нативных расширений при запуске
...
Внешние значения передаются по protocol 18 как stable type object ID, версия
...
В RadixDB 1.2.25 нет hot reload и команды обновления на месте
...
`CASCADE` для объектов расширения в 1.2.25 не поддерживается. `RESTRICT` запрещает
