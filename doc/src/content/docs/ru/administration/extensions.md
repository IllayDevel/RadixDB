...
RadixDB 1.2.21 загружает доверенные пакеты нативных расширений при запуске
...
В RadixDB 1.2.21 нет hot reload и команды обновления на месте
...
`CASCADE` для объектов расширения в 1.2.21 не поддерживается. `RESTRICT` запрещает
