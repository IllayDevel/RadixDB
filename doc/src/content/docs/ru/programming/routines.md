...
Принятые tests 1.2.21 пересобирают routines из source после persistent reopen.
...
Необязательный clause `RESOURCE POLICY` в 1.2.21 принимает только встроенное имя
...
protocol 18 передаёт только coarse `SqlError` либо `AuthorizationDenied` и
