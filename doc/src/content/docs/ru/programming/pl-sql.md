...
**RadixDB PL** является встроенным процедурным языком RadixDB 1.2.19. В catalog
...
Scalar types: `INTEGER`, `FLOAT`, `DOUBLE PRECISION`, `TEXT`, `BOOLEAN`,
`TIMESTAMPTZ`, `TIMESTAMP`, `TIME`, `JSON`, `VECTOR`, `UUID`, `DECIMAL`, `DATE`
и `BYTES`. Variables nullable по умолчанию; для запрета NULL нужен `NOT NULL`.
`CONSTANT` требует initializer.
...
Capacity задаётся при компиляции в диапазоне `1..=65536`. В 1.2.19 доступны
...
Allowlist 1.2.19 содержит один query, `INSERT`, `UPDATE`, `DELETE` или `CALL`,
...
зафиксированной базе 1.2.19. Далее: [«Функции и процедуры»](../routines/).
