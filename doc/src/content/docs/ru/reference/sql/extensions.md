...
входят в grammar 1.2.21. `IF NOT EXISTS` успешен только тогда, когда существующий
...
Database остается привязанной к записанным version и fingerprint. В 1.2.21 нет
...
Rust SDK 1.2.21 не публикует generic SQL text input/output callbacks. Создавайте
...
descriptor. Native aggregate, window и table-valued functions в 1.2.21 не
...
hash и bitmap classes требуют `=`. Authoring SDK Rust 1.2.21 отклоняет external
