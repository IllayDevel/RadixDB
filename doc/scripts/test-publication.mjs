...
try {
  ...
  const canonicalRoot = path.join(artifact, 'manual/1.2.25');
  for (const file of [
    'en/index.html',
    'ru/index.html',
  ...
  assert(sitemap.includes('https://docs.invalid/preview/manual/1.2.25/en/'));
  assert(sitemap.includes('https://docs.invalid/preview/manual/1.2.25/ru/'));
  ...
  for (const redirect of registry.redirects) for (const route of redirect.routes) {
    ...
    const target = `/preview/manual/1.2.25/${redirect.target}`;
  ...
  const asset = page.match(/(?:href|src)="(\/preview\/manual\/1\.2\.25\/_astro\/[^"]+)"/)?.[1];
  ...
  assert.equal(documentationPages.length, 151);
